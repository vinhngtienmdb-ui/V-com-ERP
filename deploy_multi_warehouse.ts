...
async function run() {
  console.log('=== VComm ERP Multi-Warehouse DDL Deployment ===');
  ...
  try {
    ...
    const sqlPath = 'C:\\Users\\vinhn\\.gemini\\antigravity\\brain\\1b6b94e2-e93f-446e-a263-b71a47b3375d\\scratch\\setup_multi_warehouse.sql';
    if (!fs.existsSync(sqlPath)) {
      throw new Error(`SQL file not found at ${sqlPath}`);
    }

    const sql = fs.readFileSync(sqlPath, 'utf8');
    console.log('Executing setup_multi_warehouse.sql...');
    await client.query(sql);
    console.log('Executed setup_multi_warehouse.sql successfully!');
