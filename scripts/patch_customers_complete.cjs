...
// 1. Replace lucide-react imports using index-based search
const lucideStart = content.indexOf("Users,");
if (lucideStart === -1) {
  console.error("Could not find Users, in content");
  process.exit(1);
}
const importIndex = content.lastIndexOf("import {", lucideStart);
if (importIndex === -1) {
  console.error("Could not find import { before Users,");
  process.exit(1);
}
const endOfImport = content.indexOf("} from 'lucide-react';", lucideStart) + "} from 'lucide-react';".length;
if (endOfImport === -1) {
  console.error("Could not find } from 'lucide-react'; after Users,");
  process.exit(1);
}

const newLucideImports = `import { 
  Users, 
  MessageSquare, 
  Facebook, 
  PhoneCall, 
...
content = content.substring(0, importIndex) + newLucideImports + content.substring(endOfImport);
console.log('Successfully replaced lucide imports');

// 2. Replace CustomerDetailModal component by index
...
content = content.substring(0, startIndex) + newCustomerDetailModal + content.substring(endIndex);
console.log('Successfully replaced CustomerDetailModal component structure');

// 3. Inject states inside export function Customers()
const stateTarget = `const [syncingCustomerId, setSyncingCustomerId] = useState<string | null>(null);`;
const stateReplacement = `const [syncingCustomerId, setSyncingCustomerId] = useState<string | null>(null);
...
// 4. Update useEffect block (without Deals)
...
if (content.match(useEffectRegex)) {
  ...
  console.log('Successfully updated useEffect block');
} else {
  console.error('Could not find useEffect block matching regex');
  process.exit(1);
}

// 5. Update CustomerDetailModal instantiation inside Customers component return
const detailModalRenderRegex = /customer=\{selectedCustomer\}\s*onClose=\{\(\)\s*=>\s*setSelectedCustomer\(null\)\}\s*\/>/;
...
if (content.match(detailModalRenderRegex)) {
  content = content.replace(detailModalRenderRegex, newDetailModalRender);
  console.log('Successfully replaced CustomerDetailModal render instantiation');
} else {
  console.error('Could not find CustomerDetailModal render instantiation');
  process.exit(1);
}

fs.writeFileSync(targetFile, content, 'utf8');
console.log('CRM completed integrations successfully applied to Customers.tsx');
