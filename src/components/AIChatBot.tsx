  ...
  return (
    <>
      <button 
        ...
        className="fixed bottom-6 right-6 p-4 bg-[#111827] text-white rounded-full shadow-2xl hover:bg-slate-800 transition-all z-[1000] active:scale-95"
      >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={cn("flex", m.role === 'user' ? "justify-end" : "justify-start")}>
                 <div className={cn("max-w-[80%] p-4 rounded-lg text-xs leading-relaxed", m.role === 'user' ? "bg-blue-600 text-white rounded-tr-none" : "bg-slate-100 text-[#111827] rounded-tl-none")}>
                   {m.content}
                 </div>
              </div>
            ))}
