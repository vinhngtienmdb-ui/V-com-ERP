...
export function AIOperations() {
 ...
 <h1 className="font-serif tracking-tight text-3xl font-black text-slate-900 tracking-tight">Trung tâm Vận hành AI <span className="text-orange-700">(AIOps)</span></h1>
 ...
 <div className="p-1.5 bg-slate-900 rounded-none shadow-sm shadow-slate-900/5 group-hover:scale-110 transition-transform duration-500">
 <Cpu className="w-4 h-4" />
 </div>
 <div className="flex flex-col items-end">
 <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Sức khỏe Hệ thống</span>
 <span className="text-xs font-bold text-emerald-400">99.99%</span>
 </div>
 </div>
 <div>
 <div className="text-base font-black tracking-tight">12 Tác nhân</div>
 <p className="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-widest">Các Nút Nơ-ron Hoạt động</p>
 </div>
 </div>
 ...
 { label: 'Cảnh báo rủi ro', value: '12', sub: 'Phát hiện hôm nay', icon: AlertTriangle, color: 'rose', alert: true },
 { label: 'Độ trễ suy luận', value: '140ms', sub: 'Thời gian phản hồi', icon: Activity, color: 'blue' },
 ].map((stat) => (
 ...
 <div className={cn(
 "p-2 rounded-none transition-transform group-hover:scale-110 duration-500",
 ...
 <div>
 <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-0.5">{stat.label}</p>
 <div className={cn("text-lg font-black tracking-tight", stat.alert ? "text-rose-600" : "text-slate-900")}>
 {stat.value}
 </div>
 ...
 { id: 'review', label: 'Người duyệt thủ công', icon: UserCheck }
 ].map((tab) => (
 <button 
 key={tab.id}
 ...
 className="bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-3 py-1.5 text-sm focus:outline-none w-full md:w-56 ring-orange-600/10 focus:ring-4 transition-all"
 />
 </div>
 <button className="bg-white border border-slate-300 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 flex items-center gap-2 hover:bg-slate-50 transition-all">
 ...
 <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]" /> Luồng suy luận trực tiếp
 </div>
 </div>
 ...
 <div key={item.id} className="p-3 bg-amber-50/30 border border-amber-200 rounded-xl flex flex-col sm:flex-row items-start gap-3 group">
 <div className="p-2 rounded-lg bg-white shadow-sm shadow-amber-200/50 text-amber-600 border border-amber-100 group-hover:scale-105 transition-transform shrink-0">
 ...
 {item.id}
 <span className="text-[10px] bg-amber-600 text-[#FAF9F5] px-2 py-0.5 rounded-full uppercase tracking-[0.1em] font-black shadow-sm shadow-amber-600/20">Chờ duyệt</span>
 </h4>
 ...
 <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">Độ tin cậy: {(item.confidence * 100).toFixed(0)}%</span>
 </div>
 </div>
 <div className="flex gap-1.5">
 <button className="px-3 py-1 bg-white border border-slate-300 text-rose-600 font-black rounded-lg text-[10px] uppercase tracking-[0.1em] hover:bg-rose-50 transition-all border-b-2 active:translate-y-0.5">Từ chối</button>
 <button className="px-3 py-1 bg-slate-900 text-[#FAF9F5] font-black rounded-lg text-[10px] uppercase tracking-[0.1em] hover:bg-emerald-600 transition-all shadow-sm shadow-slate-900/20 active:translate-y-0.5">Duyệt</button>
 </div>
 </div>
 <div className="p-2 bg-white/80 rounded-lg border border-amber-200/50 shadow-inner">
 <p className="text-xs font-bold text-amber-900/80 leading-relaxed italic">"{item.reason}"</p>
 </div>
 </div>
 </div>
 ))
 ...
 <div key={log.id} className="p-3 bg-white border border-slate-200 rounded-xl hover:border-orange-200 hover:shadow-sm hover:shadow-slate-900/5 transition-all group flex flex-col sm:flex-row items-start gap-3 border-b-2">
 <div className={cn(
 "p-2 rounded-lg shadow-sm transition-all group-hover:scale-105 shrink-0",
 log.status === 'flagged' ? "bg-rose-50 text-rose-500 shadow-rose-200/30" : "bg-emerald-50 text-emerald-500 shadow-emerald-200/30"
 )}>
 {log.status === 'flagged' ? <AlertTriangle className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
 </div>
 <div className="flex-1 space-y-2">
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
 <div>
 <h4 className="text-sm font-black text-slate-900 flex items-center gap-2 italic">
 {log.id} 
 ...
 )}>Độ tin cậy: {(log.confidence * 100).toFixed(0)}%</span>
 </h4>
 <div className="flex items-center gap-2 mt-1">
 <span className="text-[10px] text-slate-600 font-bold flex items-center gap-1"><Clock className="w-3 h-3" /> {log.timestamp}</span>
 <div className="w-px h-3 bg-slate-300" />
 <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest opacity-60">Đối tượng: {log.targetId}</span>
 </div>
 </div>
 <div className="flex gap-1.5">
 <button className="px-3 py-1 bg-slate-100 text-slate-900 font-black rounded-lg text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border-b-2 active:translate-y-0.5">Ghi đè</button>
 <button className="px-3 py-1 bg-slate-900 text-[#FAF9F5] font-black rounded-lg text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm shadow-blue-200 active:translate-y-0.5">Nhật ký</button>
 </div>
 </div>
 <div className="p-2 bg-slate-50 rounded-lg border border-slate-200/50 shadow-inner group-hover:bg-slate-100/50 transition-colors">
 <p className="text-xs font-bold text-slate-800 leading-relaxed">
 <span className="text-orange-700 uppercase tracking-widest text-[10px] font-black mr-2">Kết quả xử lý:</span> {renderResult(log)}
 </p>
 </div>
 </div>
 </div>
 ...
 <DollarSign className="w-6 h-6 text-emerald-500" /> Chi phí & Hiệu suất Token
 </h3>
 ...
 <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest mb-1">Tỉ lệ Cache trúng</p>
 ...
 <div className="flex justify-between items-center mb-4">
 <h3 className="text-base font-black text-slate-900 flex items-center gap-3">
 <LineChart className="w-6 h-6 text-orange-700" /> Đấu trường So sánh Model
 </h3>
 <button className="text-[10px] font-black text-orange-700 uppercase tracking-widest px-4 py-2 bg-slate-100 rounded-xl border border-slate-300 hover:bg-[#EAE7DF] transition-all">Bắt đầu A/B Test</button>
 </div>
 <div className="overflow-x-auto min-w-0">
 <table className="w-full text-left">
 <thead>
 <tr className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-slate-200">
 <th className="pb-2">Model AI</th>
 <th className="pb-2">Độ trễ</th>
 <th className="pb-2 text-right">Độ chính xác</th>
 </tr>
 </thead>
 ...
 <td className="py-2 font-black text-slate-900 text-xs">{m.name} {m.highlight && <span className="ml-2 px-2 py-0.5 bg-slate-900 text-[#FAF9F5] text-[8px] rounded uppercase tracking-widest">Đang dùng</span>}</td>
 <td className="py-2 text-slate-600 font-bold text-xs"><Clock className="w-3.5 h-3.5 inline mr-1 text-slate-500" /> {m.latency}</td>
 <td className="py-2 text-right font-mono font-black text-xs text-emerald-600">{m.accuracy}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="p-2 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10 shadow-sm group-hover:scale-110 transition-transform duration-500">
 <BarChart4 className="w-5 h-5 text-orange-500" />
 </div>
 <h3 className="text-base font-black italic font-serif">MoE Intelligence <span className="text-orange-600">Router</span></h3>
 </div>
 ...
 <button className="px-3 py-1.5 bg-slate-900 text-[#FAF9F5] font-black rounded-lg text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm shadow-slate-900/5 hover:-translate-y-1">Nhật ký Suy luận</button>
 <button className="px-3 py-1.5 bg-slate-900/5 backdrop-blur-md border border-white/10 text-[#FAF9F5] font-black rounded-lg text-xs uppercase tracking-widest hover:bg-slate-900/10 transition-all">Chính sách Tối ưu</button>
 </div>
 </div>
 ...
 <span className={cn(
 "text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shadow-sm",
 m.statusColor
 )}>{m.health}</span>
 </div>
 ...
 <span className={cn(m.drift < 0 ? "text-rose-500" : "text-emerald-600")}>Độ lệch chính xác: {m.drift}%</span>
 <span>Uptime node: 99.99%</span>
 </div>
 </div>
 ))}
 <button className="w-full py-2.5 bg-slate-50 text-slate-600 text-xs font-black rounded-[1.5rem] border border-slate-200 hover:text-orange-700 hover:bg-slate-100 transition-all uppercase tracking-widest border-b-4 active:translate-y-1">Xem toàn bộ Kiến trúc AI</button>
 </div>
 </div>
 </DraggableGrid>
 ...
 <span className="p-1.5 bg-emerald-50 rounded-lg text-emerald-600 shadow-sm shadow-emerald-500/10 inline-block"><Server className="w-4 h-4" /></span>
 Giám sát Cụm Phần cứng
 </h3>
 <p className="text-slate-600 text-xs font-medium mt-1">Giám sát tải các cụm GPU NVIDIA dành cho suy luận & tinh chỉnh model.</p>
 </div>
 <button className="px-3 py-1.5 border border-emerald-200 text-emerald-700 font-black text-[11px] uppercase tracking-widest rounded-xl hover:bg-emerald-50 transition-all flex items-center gap-2 shadow-sm shadow-emerald-700/5 bg-white border-b-4 active:translate-y-1">
 <RefreshCw className="w-3.5 h-3.5" /> Đồng bộ Cụm
 </button>
 </div>
 ...
 <span className="text-[9px] bg-emerald-600 text-[#FAF9F5] px-3 py-1 rounded-full font-black uppercase tracking-widest shadow-sm shadow-emerald-600/20">Nhóm Huấn luyện</span>
 </div>
 <div className="space-y-3">
 <div className="space-y-2">
 <div className="flex justify-between text-[10px] font-black text-slate-600 uppercase tracking-[0.1em]">
 <span>Mức dùng VRAM</span>
 <span className="text-rose-600">92% Nguy hiểm</span>
 </div>
 ...
 <div className="flex justify-between text-[10px] font-black text-slate-600 uppercase tracking-[0.1em]">
 <span>Tải GPU</span>
 <span className="text-orange-500">85% Cao</span>
 </div>
 <div className="h-2 bg-slate-200 rounded-full overflow-hidden shadow-inner flex">
 ...
 <span className="w-2 h-2 bg-slate-800 rounded-full animate-pulse" /> Đang tinh chỉnh NLP v4.0
 </p>
 </div>
 </div>
 </div>
 ...
 <span className="text-[9px] bg-slate-900 text-[#FAF9F5] px-3 py-1 rounded-full font-black uppercase tracking-widest shadow-sm shadow-slate-900/5">Nhóm Suy luận</span>
 </div>
 <div className="space-y-3">
 <div className="space-y-2">
 <div className="flex justify-between text-[10px] font-black text-slate-600 uppercase tracking-[0.1em]">
 <span>Mức dùng VRAM</span>
 <span className="text-orange-700">45% Ổn định</span>
 </div>
 ...
 <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest flex items-center gap-2">
 <span className="w-2 h-2 bg-emerald-500 rounded-full" /> Kiểm duyệt hình ảnh đang chạy
 </p>
 </div>
 </div>
 </div>

 <div className="border-2 border-slate-200 border-dashed rounded-xl p-4 bg-slate-50/50 flex flex-col items-center justify-center min-h-[150px] hover:bg-slate-100/50 hover:border-blue-300 hover:text-orange-700 transition-all cursor-pointer group/add">
 <div className="p-3 bg-white rounded-full shadow-sm group-hover/add:scale-110 transition-transform mb-2 border border-slate-200">
 <Plus className="w-5 h-5 text-slate-500 group-hover/add:text-orange-600" />
 </div>
 <span className="font-black text-xs uppercase tracking-[0.2em] text-slate-500 group-hover/add:text-orange-700">Thêm Cụm Máy chủ</span>
 <span className="text-[10px] font-bold text-slate-500 mt-1">Azure / AWS / GCP</span>
 </div>
 </DraggableGrid>
 </div>
 </div>
