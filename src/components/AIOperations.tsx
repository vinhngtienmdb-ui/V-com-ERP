...
export function AIOperations() {
 ...
 return (
 <div className="space-y-4 animate-in fade-in slide-in- duration-700 pb-4">
 <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
 <div className="header-title">
 <div className="flex items-center gap-2 mb-2">
 <span className="text-[10px] font-bold uppercase tracking-widest text-orange-500 bg-blue-950/50 border border-blue-900 px-2 py-0.5 rounded-none">Trung tâm Mạng Nơ-ron</span>
 <div className="w-1.5 h-1.5 bg-slate-800 rounded-full animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.6)]" />
 </div>
 <h1 className="font-sans tracking-tight text-lg font-bold text-slate-900 tracking-tight">AI Operations <span className="text-blue-600">(AIOps)</span></h1>
 ...
 <button className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-none text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm shadow-slate-900/20 flex items-center gap-2 hover:scale-[1.02] active:scale-95">
 ...
 className={cn(
 "px-6 py-2 text-xs font-bold uppercase tracking-widest transition-all flex items-center gap-2 rounded-none",
 activeModel === tab.id ? "bg-blue-600 text-white shadow-sm shadow-slate-900/5" : "text-slate-600 hover:text-slate-900 hover:bg-slate-50"
 )}
 >
 ...
 <button className="bg-white border border-slate-300 px-5 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest text-slate-600 flex items-center gap-2 hover:bg-slate-50 transition-all">
 <Filter className="w-4 h-4" /> Filter Confidence
 </button>
 </div>
 <div className="flex items-center gap-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-full border border-slate-200">
 ...
 <div key={item.id} className="p-4 bg-amber-50/30 border border-amber-200 rounded-xl flex flex-col sm:flex-row items-start gap-3 group">
 ...
 {item.id}
 <span className="text-[10px] bg-amber-600 text-white px-3 py-1 rounded-full uppercase tracking-[0.1em] font-bold shadow-sm shadow-amber-600/20">Chờ người duyệt</span>
 </h4>
 ...
 <button className="px-6 py-3 bg-white border border-slate-300 text-rose-600 font-bold rounded-xl text-[10px] uppercase tracking-[0.1em] hover:bg-rose-50 transition-all border-b-2 active:translate-y-0.5">Reject Action</button>
 <button className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-xl text-[10px] uppercase tracking-[0.1em] hover:bg-emerald-600 transition-all shadow-sm shadow-slate-900/20 active:translate-y-0.5">Approve Model</button>
 </div>
 </div>
 ...
 <div key={log.id} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-orange-200 hover:shadow-sm hover:shadow-slate-900/5 transition-all group flex flex-col sm:flex-row items-start gap-3 border-b-2">
 <div className={cn(
 ...
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
 <div>
 <h4 className="text-lg font-bold text-slate-900 flex items-center gap-3 italic">
 {log.id} 
 <span className="w-1.5 h-1.5 bg-slate-300 rounded-full" />
 <span className={cn(
 "text-[10px] px-3 py-1 rounded-full uppercase tracking-widest font-bold font-sans",
 ...
 <button className="px-5 py-2.5 bg-slate-100 text-slate-900 font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border-b-2 active:translate-y-0.5">Override</button>
 <button className="px-5 py-2.5 bg-blue-600 text-white font-semibold rounded-xl text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm shadow-blue-200 active:translate-y-0.5">Telemetry</button>
 </div>
 </div>
 <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200/50 shadow-inner group-hover:bg-slate-100/50 transition-colors">
 <p className="text-sm font-bold text-slate-800 leading-relaxed">
 <span className="text-blue-600 uppercase tracking-widest text-[10px] font-bold mr-2">Action Report:</span> {renderResult(log)}
 </p>
 </div>
 </div>
 </div>
 ...
 <DraggableGrid className="grid grid-cols-1 md:grid-cols-2 gap-3" columns={2} gap={32}>
 <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm shadow-slate-200/40">
 ...
 <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Ngân sách Hàng tháng</span>
 </div>
 <DraggableGrid className="grid grid-cols-2 gap-3 mb-4" columns={2} gap={24}>
 <div className="p-3 bg-stone-950 text-white rounded-xl border border-slate-800 shadow-sm relative overflow-hidden group">
 <p className="text-[10px] text-orange-500 font-bold uppercase tracking-widest mb-2">Chi phí Thời gian thực</p>
 <p className="text-lg font-bold tracking-tight tracking-tighter">31,012,500đ</p>
 <div className="absolute top-0 right-0 w-8 h-8 bg-slate-900/10 rounded-full blur-xl group-hover:bg-slate-900/20 transition-all" />
 </div>
 <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 shadow-sm shadow-emerald-500/10 group">
 <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest mb-2">Cache Hit Rate</p>
 <p className="text-lg font-bold text-emerald-700 tracking-tight tracking-tighter">32.4%</p>
 ...
 <span className="font-bold text-slate-800 uppercase tracking-widest flex items-center gap-2">
 <Globe className="w-3.5 h-3.5 text-blue-600" /> {p.provider}
 </span>
 ...
 <button className="text-[10px] font-bold text-blue-600 uppercase tracking-widest px-4 py-2 bg-slate-100 rounded-xl border border-slate-300 hover:bg-[#EAE7DF] transition-all">Start A/B Battle</button>
 </div>
 ...
 <td className="py-5 font-bold text-slate-900 text-sm">{m.name} {m.highlight && <span className="ml-2 px-2 py-0.5 bg-blue-600 text-white text-[8px] rounded uppercase tracking-widest">Selected</span>}</td>
 <td className="py-5 text-slate-600 font-bold text-xs"><Clock className="w-3.5 h-3.5 inline mr-1 text-slate-500" /> {m.latency}</td>
 <td className="py-5 text-right font-mono font-bold text-sm text-emerald-600">{m.accuracy}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <DraggableGrid className="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-4" columns={2} gap={32}>
 <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm group">
 <div className="absolute top-0 right-0 w-10 h-10 bg-slate-900/10 rounded-full blur-[100px] -mr-32 -mt-32 group-hover:bg-slate-900/20 transition-all duration-1000" />
 ...
 <h3 className="text-xl font-bold tracking-tight leading-tight italic font-sans">MoE Intelligence <br /> <span className="text-blue-600">Router</span></h3>
 </div>
 <p className="text-slate-500 text-[12px] font-medium leading-relaxed max-w-md">
 ...
 <button className="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm shadow-slate-900/5 hover:-translate-y-1">Inference Logs</button>
 <button className="px-4 py-2 bg-slate-900/5 backdrop-blur-md border border-white/10 text-white font-bold rounded-lg text-xs uppercase tracking-widest hover:bg-slate-900/10 transition-all">Optimization Policy</button>
 </div>
 </div>
 <Layers className="absolute -bottom-16 -right-16 w-10 h-10 text-white/5 opacity-50 group-hover:rotate-12 transition-transform duration-1000" />
 </div>

 <div className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm shadow-slate-200/40 space-y-4">
 <h3 className="text-xl font-bold text-slate-900 flex items-center gap-4">
 <Activity className="w-7 h-7 text-blue-600" /> Phân tích Độ lệch & Tình trạng Model
 </h3>
 <div className="space-y-4">
 {[
 ...
 { name: 'Dynamic Pricing Agent', drift: 0.1, health: 'Stable', statusColor: 'bg-slate-100 text-blue-600' }
 ].map((m, i) => (
 ...
 <button className="w-full py-5 bg-slate-50 text-slate-600 text-xs font-bold rounded-[1.5rem] border border-slate-200 hover:text-blue-600 hover:bg-slate-100 transition-all uppercase tracking-widest border-b-4 active:translate-y-1">View Full Neural Architecture</button>
 </div>
 </div>
 </DraggableGrid>

 {/* Hardware & Resource Grid */}
 <div className="mt-3 bg-white rounded-xl border border-slate-200 shadow-sm p-4 relative overflow-hidden group">
 ...
 <Server className="w-10 h-10 rotate-12" />
 </div>
 
 <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-12 relative z-10">
 <div>
 ...
 <button className="px-6 py-3 border border-emerald-200 text-emerald-700 font-bold text-[11px] uppercase tracking-widest rounded-xl hover:bg-emerald-50 transition-all flex items-center gap-3 shadow-sm shadow-emerald-700/5 bg-white border-b-4 active:translate-y-1">
 <RefreshCw className="w-4 h-4" /> Sync Cluster State
 </button>
 </div>

 <DraggableGrid className="grid grid-cols-1 md:grid-cols-3 gap-3 relative z-10" columns={3} gap={32}>
 ...
 <h4 className="font-bold text-slate-900 text-lg uppercase tracking-tight">Node: A100-80GB</h4>
 <span className="text-[9px] bg-emerald-600 text-white px-3 py-1 rounded-full font-bold uppercase tracking-widest shadow-sm shadow-emerald-600/20">Training Pool</span>
 </div>
 <div className="space-y-3">
 <div className="space-y-2">
 <div className="flex justify-between text-[10px] font-bold text-slate-600 uppercase tracking-[0.1em]">
 ...
 <div className="flex justify-between items-center mb-3">
 <h4 className="font-bold text-slate-900 text-lg uppercase tracking-tight">Node: L4-24GB</h4>
 <span className="text-[9px] bg-blue-600 text-white px-3 py-1 rounded-full font-bold uppercase tracking-widest shadow-sm shadow-slate-900/5">Inference Pool</span>
 </div>
 <div className="space-y-3">
 <div className="space-y-2">
 <div className="flex justify-between text-[10px] font-bold text-slate-600 uppercase tracking-[0.1em]">
 ...
 <div className="border-2 border-slate-200 border-dashed rounded-xl p-4 bg-slate-50/50 flex flex-col items-center justify-center hover:bg-slate-100/50 hover:border-blue-300 hover:text-blue-600 transition-all cursor-pointer group/add">
 ...
 <Plus className="w-10 h-10 text-slate-500 group-hover/add:text-blue-600" />
 </div>
 <span className="font-bold text-xs uppercase tracking-widest text-slate-500 group-hover/add:text-blue-600">Deploy New Node</span>
