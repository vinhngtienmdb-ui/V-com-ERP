    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex items-center justify-between">
        <div className="header-title">
          <h1 className="text-2xl font-semibold text-[#111827]">Advertising Manager (Quảng cáo nội bộ)</h1>
                  ...
                  {MOCK_BIDS.map(bid => (
                    <tr key={bid.id} className="hover:bg-slate-50 transition-colors text-xs">
                      <td className="px-6 py-4">
                         ...
                         <p className="text-[10px] text-slate-500 font-mono">Seller ID: {bid.sellerId}</p>
                      </td>
                      <td className="px-6 py-4">
                         <div className="space-y-1">
                            <span className={cn(
                              "px-2 py-0.5 rounded text-[9px] font-bold uppercase",
                              bid.type === 'keyword' ? "bg-blue-50 text-blue-600" :
                              bid.type === 'banner' ? "bg-purple-50 text-purple-600" : "bg-emerald-50 text-emerald-600"
                            )}>
                               {bid.type.replace('_', ' ')}
                            </span>
                            <p className="text-[10px] font-bold text-slate-700">{bid.target}</p>
                         </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <p className="font-bold text-[#111827]">{formatCurrency(bid.bidAmount)}</p>
                        <p className="text-[10px] text-slate-400">trên {bid.type === 'keyword' ? 'Click' : '1000 Imp'}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="max-w-[120px] space-y-1.5">
                           ...
                           <p className="text-[9px] text-slate-400 text-right">Hạn mức: {formatCurrency(bid.budget)}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <p className="font-bold text-[#111827]">{bid.clicks.toLocaleString()}</p>
                        <p className="text-[10px] text-slate-400">{bid.impressions.toLocaleString()} views</p>
                      </td>
                      <td className="px-6 py-4">
                     ...
                     <div className="bg-white p-6 rounded-lg border border-[#E5E7EB] shadow-sm h-full flex flex-col justify-center">
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-50 space-y-3">
                           <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-500">Từ khóa (Keywords)</span>
                           ...
                           <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-500">Banners & Ads</span>
               ...
               <div className="bg-white p-6 rounded-lg border border-[#E5E7EB] shadow-sm">
                  ...
                  <div className="space-y-4">
                     {SELLER_PERFORMANCE.sort((a, b) => b.ctr - a.ctr).map((seller, i) => (
                        <div key={seller.name} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group">
                           <div className="flex items-center gap-3">
                              ...
                              <div>
                                 ...
                                 <p className="text-[10px] text-slate-400">Campaign Accuracy: High</p>
                              </div>
                           </div>
                  ...
                  <button className="w-full py-3 mt-6 border border-slate-100 rounded-lg text-[10px] font-bold text-slate-400 hover:bg-slate-50 hover:text-blue-600 transition-all uppercase tracking-widest">
