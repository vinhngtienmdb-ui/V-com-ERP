...
export function AffiliateManagement() {
 ...
 <button className="bg-white border border-[#E5E7EB] px-4 py-2 rounded-lg text-sm font-medium hover:bg-stone-50 transition-all flex items-center gap-2 shadow-sm text-stone-700">
 ...
 className={cn("px-4 py-2 text-xs font-semibold border-l border-[#E5E7EB]", activeTab === 'pending' ? "bg-[#2563EB] text-[#FAF9F5]" : "text-[#4B5563]")}
 ...
 <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center text-[#2563EB] font-bold text-xs border border-[#E5E7EB] shrink-0">
 ...
 <span className="text-[10px] text-orange-700 bg-[#F2F0E9] px-1 rounded-sm ml-1">{affiliate.categoryTags[0]}</span>
 )}
 </div>
 </div>
 </div>
 ...
 <span className="text-xs text-stone-700 capitalize">{affiliate.platforms?.join(', ')}</span>
 </div>
 ...
 <p className="text-[10px] text-stone-500 mt-1">Booking: {formatCurrency(affiliate.bookingPrice)}</p>
 )}
 </td>
 <td className="px-6 py-4">
 ...
 affiliate.status === 'active' ? "bg-emerald-50 text-emerald-600" : "bg-[#F2F0E9] text-orange-700"
 )}>
 ...
 <button className="px-3 py-1.5 bg-[#2563EB] text-[#FAF9F5] text-[11px] font-bold rounded-md hover:bg-stone-800 shadow-sm">Duyệt KOL</button>
 ) : (
