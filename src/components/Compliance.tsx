    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex items-center justify-between">
        <div className="header-title">
          <h1 className="text-2xl font-semibold text-[#111827]">Pháp chế & Bảo vệ thương hiệu</h1>
              ...
              {activeTab === 'brand' && MOCK_BRANDS.map(brand => (
                <tr key={brand.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                     ...
                     <p className="text-[10px] text-slate-500 font-mono">Owner: {brand.ownerId}</p>
                  </td>
                  <td className="px-6 py-4 text-xs font-mono text-slate-600">{brand.id}</td>
                  <td className="px-6 py-4 text-xs text-slate-500">{brand.registrationDate}</td>
                  <td className="px-6 py-4">
              ...
              {activeTab === 'dispute' && MOCK_DISPUTES.map(dispute => (
                <tr key={dispute.id} className="hover:bg-slate-50 transition-colors text-xs">
                  ...
                  <td className="px-6 py-4">
                     ...
                     <p className="text-[10px] text-slate-500">Người báo: {dispute.reporterId}</p>
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                     <span className={cn(
                       ...
                       dispute.status === 'investigating' ? "bg-blue-50 text-blue-600" : "bg-red-50 text-red-600"
                     )}>
         ...
         <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="space-y-4">
               <div className="flex items-center gap-3">
                  <div className="p-3 bg-red-600 rounded-lg shadow-xl shadow-red-600/20">
                  ...
                  <div className="space-y-3">
                     {[1, 2].map(i => (
                        <div key={i} className="flex gap-3 text-xs border-l-2 border-red-500 pl-4 py-1">
                           <div>
                              <p className="text-slate-200 font-bold">Phát hiện Seller bán hàng giả mạo (Counterfeit)</p>
                              <p className="text-slate-500 text-[10px]">Mã shop: SEL-0{i}42 • 5 phút trước</p>
                           </div>
                        </div>
                     ))}
                  </div>
