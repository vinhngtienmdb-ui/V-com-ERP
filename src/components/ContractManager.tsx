...
export function ContractManager() {
 ...
 {selectedContract && (
 <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/50 backdrop-blur-sm animate-in fade-in">
 ...
 <div>
 <h3 className="text-lg font-bold text-stone-800">{selectedContract.title}</h3>
 <p className="text-sm font-mono text-stone-500 mt-1">{selectedContract.id}</p>
 </div>
 <button 
 ...
 <div>
 <p className="text-sm text-stone-500">Đối tác / Nhân sự</p>
 <p className="text-base font-medium text-stone-800 mt-1">{selectedContract.party}</p>
 </div>
 <div>
 <p className="text-sm text-stone-500">Loại hợp đồng</p>
 <p className="text-base font-medium text-stone-800 mt-1">{selectedContract.subtype || selectedContract.type}</p>
 </div>
 <div>
 <p className="text-sm text-stone-500">Giá trị hợp đồng</p>
 <p className="text-base font-medium text-stone-800 mt-1">{selectedContract.value}</p>
 </div>
 <div>
 <p className="text-sm text-stone-500">Ngày hết hạn</p>
 <p className={cn(
 "text-base font-bold mt-1",
 selectedContract.status === 'expired' ? "text-red-600" :
 selectedContract.status === 'expiring_soon' ? "text-orange-600" : "text-stone-800"
 )}>{selectedContract.expiry}</p>
 </div>
 </div>

 <div>
 <p className="text-sm text-stone-500 mb-2">Trạng thái hợp đồng</p>
 ...
 <span className={cn(
 "px-3 py-1.5 text-sm font-bold rounded-lg uppercase tracking-tight inline-flex items-center gap-2",
 selectedContract.signatureStatus === 'signed' ? "bg-[#F2F0E9] text-orange-700" : "bg-stone-100 text-stone-600"
 )}>
 ...
 <div>
 <p className="text-sm font-bold text-stone-800">{signer.name}</p>
 <p className="text-xs text-stone-500">{signer.role}</p>
 </div>
 <div>
 ...
 <span className="flex items-center gap-1.5 text-xs font-bold text-stone-500 bg-stone-100 px-2 py-1 rounded">
 ...
 className="px-4 py-2 bg-stone-900 text-[#FAF9F5] rounded-lg text-sm font-semibold hover:bg-stone-800 shadow-sm flex items-center gap-2"
 >
 ...
 <button 
 onClick={() => setSigningModalOpen(false)}
 className="px-4 py-2.5 text-sm font-medium text-stone-600 hover:text-stone-800 hover:bg-stone-100 rounded-lg transition-colors"
 >
 Hủy bỏ
 </button>
 <button 
 ...
 <tr key={doc.id} onClick={() => setSelectedContract(doc)} className="hover:bg-stone-50 transition-colors cursor-pointer">
 ...
 <td className="px-6 py-4">
 <p className="text-sm font-medium text-stone-800">{doc.party}</p>
 </td>
 <td className="px-6 py-4">
 <p className="text-sm font-bold text-stone-700">{doc.value}</p>
 </td>
 ...
 doc.signatureStatus === 'signed' ? "bg-[#F2F0E9] text-orange-700" : "bg-stone-100 text-stone-600"
 )}>
 ...
 doc.status === 'expiring_soon' ? "text-orange-600 font-bold" : "text-stone-600"
 )}>{doc.expiry}</p>
 </td>
 </tr>
 ))}
