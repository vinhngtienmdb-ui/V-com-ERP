...
const StatCard = ({ title, value, change, icon: Icon, trend, subValue }: any) => (
 <div className="bg-white p-5 rounded-lg border border-stone-200 shadow-sm hover:shadow-sm hover:shadow-stone-200/40 transition-all duration-300 group hover:-translate-y-1 relative overflow-hidden">
 ...
 <Icon className="w-24 h-24 -mr-6 -mt-6 text-stone-900" />
 </div>
 <div className="relative z-10">
 <div className="flex justify-between items-start mb-4">
 <div className="p-3 bg-stone-50 text-stone-600 rounded-xl group-hover:text-orange-700 group-hover:bg-[#F2F0E9] transition-colors border border-stone-100 group-hover:border-[#EAE7DF] shadow-sm">
 ...
 <div className="text-[11px] text-stone-500 font-bold uppercase tracking-wider mb-1.5">{title}</div>
 <div className="text-2xl font-black text-stone-900 tracking-tight">{value}</div>
 {subValue && (
 <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-stone-100 text-[11px] text-stone-500 font-medium">
 {subValue}
 </div>
 )}
 </div>
...
const QuickActionCard = ({ title, icon: Icon, onClick, color, description }: any) => (
 ...
 color === 'bg-stone-900' ? "bg-white border-stone-900 hover:shadow-stone-900/5" : 
 color === 'bg-emerald-600' ? "bg-white border-emerald-500 hover:shadow-emerald-500/30" : 
 "bg-white border-stone-700 hover:shadow-stone-900/30"
 )}
 >
...
export function Dashboard() {
 ...
 <div className="relative md:min-h-[14rem] bg-white rounded-xl p-6 md:p-10 text-[#FAF9F5] overflow-hidden shadow-sm shadow-stone-900/5 group">
 ...
 <div className="bg-white/10 hover:bg-white/15 p-5 md:p-6 rounded-lg border border-white/20 backdrop-blur-2xl shadow-sm w-full md:w-[340px] transition-all duration-500 group-hover:translate-y-[-4px] group-hover:shadow-stone-900/5 flex flex-col justify-center">
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-2">
 <div className="p-1 px-2 bg-stone-800/20 rounded border border-stone-900/30">
 ...
 <p className="text-xs font-semibold leading-relaxed mb-5 text-stone-100 italic">
 "SKU-992 demand increase +45% next week. Suggest stock transfer from Warehouse A to B today."
 </p>
 
 <button className="w-full py-2.5 bg-stone-900 hover:bg-stone-800 active:bg-stone-800 text-[#FAF9F5] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm shadow-stone-900/5 border border-blue-400/30 hover:scale-[1.02] active:scale-95">
 Execute AI Proposal
 </button>
 </div>
 </div>
 ...
 <span className="px-3 py-1 bg-[#F2F0E9] text-orange-800 text-[10px] font-black rounded-lg uppercase tracking-widest border border-orange-200/60 shadow-sm">Live Dashboard</span>
 <span className="text-[11px] text-stone-500 font-bold uppercase tracking-wider flex items-center gap-1.5">
 ...
 <h1 className="font-serif tracking-tight text-3xl font-black text-stone-900 tracking-tight">Tổng quan Hệ thống</h1>
 <p className="text-sm text-stone-500 mt-2 max-w-lg font-medium">Theo dõi hiệu suất đa kênh.</p>
 </div>
 
 <div className="flex gap-3">
 <button className="bg-white border border-stone-200 px-4 py-2.5 rounded-xl text-sm font-bold text-stone-700 hover:bg-stone-50 hover:text-orange-700 transition-all flex items-center justify-center gap-2 shadow-sm">
 <Activity className="w-4 h-4 text-emerald-500" />
 Báo cáo Vận hành
 </button>
 <button className="bg-stone-900 text-[#FAF9F5] px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-stone-900 transition-all shadow-sm shadow-stone-900/10 hover:shadow-stone-900/5">
 Xuất dữ liệu BI
 </button>
 <button 
 onClick={() => setIsConfigOpen(true)}
 className="bg-white border border-stone-200 px-4 py-2.5 rounded-xl text-sm font-bold text-stone-700 hover:bg-stone-50 transition-all flex items-center gap-2 shadow-sm shrink-0"
 >
 ...
 <button onClick={() => navigate('/pim')} className="bg-white border border-stone-200 px-4 py-2.5 rounded-xl text-sm font-bold text-stone-700 hover:bg-[#F2F0E9] hover:text-orange-800 transition-all flex items-center gap-2 shadow-sm group">
 <Package className="w-4 h-4 text-orange-600 group-hover:scale-110 transition-transform" />
 PIM System
 </button>
 <button onClick={() => navigate('/orders')} className="bg-white border border-stone-200 px-4 py-2.5 rounded-xl text-sm font-bold text-stone-700 hover:bg-emerald-50 hover:text-emerald-700 transition-all flex items-center gap-2 shadow-sm group">
 <ListOrdered className="w-4 h-4 text-emerald-500 group-hover:scale-110 transition-transform" />
 Order Center
 </button>
 <button onClick={() => navigate('/sellers')} className="bg-white border border-stone-200 px-4 py-2.5 rounded-xl text-sm font-bold text-stone-700 hover:bg-stone-100 hover:text-stone-900 transition-all flex items-center gap-2 shadow-sm group">
 ...
 {/* Tabs */}
 <div className="flex items-center gap-6 border-b border-stone-200 shrink-0">
 <button onClick={() => setActiveTab('overview')} className={cn("pb-3 px-1 text-sm font-bold border-b-2 transition-colors", activeTab === 'overview' ? "border-stone-900 text-orange-700" : "border-transparent text-stone-500 hover:text-stone-900")}>
 Kinh doanh & Bán hàng
 </button>
 <button onClick={() => setActiveTab('performance')} className={cn("pb-3 px-1 text-sm font-bold border-b-2 transition-colors", activeTab === 'performance' ? "border-stone-900 text-orange-700" : "border-transparent text-stone-500 hover:text-stone-900")}>
 Hiệu suất Vận hành
 </button>
 <button onClick={() => setActiveTab('finance')} className={cn("pb-3 px-1 text-sm font-bold border-b-2 transition-colors", activeTab === 'finance' ? "border-stone-900 text-orange-700" : "border-transparent text-stone-500 hover:text-stone-900")}>
 Tài chính & Dòng tiền
 </button>
 </div>
 ...
 <h3 className="font-bold text-stone-900 text-lg">Tùy chỉnh Giao diện</h3>
 </div>
 <button onClick={() => setIsConfigOpen(false)} className="p-2 text-stone-400 hover:text-stone-600 hover:bg-stone-100 rounded-lg transition-colors">
 <X className="w-5 h-5" />
 </button>
 </div>
 <div className="flex-1 overflow-y-auto p-6 bg-white custom-scrollbar">
 <p className="text-sm font-medium text-stone-500 mb-6">Chọn các Widget (thẻ Báo cáo) bạn muốn hiển thị trên biểu đồ chính, bảng xếp hạng và lối tắt.</p>
 <div className="space-y-4">
 {[
 ...
 <label key={item.id} className="flex items-center justify-between p-4 rounded-xl border border-stone-100 bg-stone-50 hover:bg-[#F2F0E9]/50 hover:border-[#EAE7DF] transition-colors cursor-pointer group">
 <div className="flex items-center gap-3">
 <item.icon className={cn("w-5 h-5", config[item.id] ? "text-orange-600" : "text-stone-400 group-hover:text-orange-500")} />
 <span className={cn("text-sm font-bold", config[item.id] ? "text-stone-900" : "text-stone-500 group-hover:text-stone-700")}>{item.label}</span>
 </div>
 <div className={cn("w-11 h-6 rounded-full p-1 transition-colors relative shadow-inner", config[item.id] ? "bg-stone-900" : "bg-stone-300")}>
 ...
 <button onClick={() => setIsConfigOpen(false)} className="px-6 py-3 w-full bg-stone-900 text-[#FAF9F5] rounded-xl text-sm font-bold hover:bg-stone-900 shadow-sm shadow-stone-900/10 hover:shadow-stone-900/5 transition-all flex justify-center items-center gap-2">
 ...
 <div className="bg-white rounded-xl border border-stone-200 shadow-sm hover:shadow-sm transition-shadow overflow-hidden flex flex-col">
 <div className="px-6 py-5 border-b border-stone-100 flex items-center justify-between bg-stone-50/50">
 <div>
 <h3 className="font-bold text-stone-900 tracking-tight text-lg">Biểu đồ Tăng trưởng & Xu hướng</h3>
 </div>
 ...
 <div className="bg-white to-[#0B1120] rounded-xl p-8 text-[#FAF9F5] relative overflow-hidden shadow-sm shadow-stone-900/10 border border-stone-800 flex flex-col justify-between">
 <div className="relative z-10">
 <div className="flex items-center gap-4 mb-6">
 <div className="p-3 bg-white rounded-lg shadow-sm shadow-stone-900/5 shadow-inner border border-blue-400/50">
 ...
 <p className="text-stone-400 text-xs leading-relaxed max-w-sm">Hơn 2,400 SKU mới đang chờ duyệt trong 24h tới.</p>
 </div>
 <div className="mt-6">
 <button className="px-5 py-2 bg-white text-stone-900 font-bold rounded-lg text-xs hover:bg-[#F2F0E9] transition-all shadow-sm">Duyệt Seller mới</button>
 </div>
 <Users className="absolute -bottom-8 -right-8 w-48 h-48 text-[#FAF9F5] opacity-[0.02]" />
 </div>
 )}
 ...
 <h3 className="font-bold text-stone-900 text-sm">Tỷ trọng Ngành</h3>
 </div>
 ...
 <div className="bg-white rounded-xl border border-stone-200 shadow-sm hover:shadow-sm transition-shadow">
 <div className="px-5 py-4 border-b border-stone-100 flex items-center justify-between bg-stone-50/50 rounded-t-xl">
 <h3 className="font-bold text-stone-900 text-sm tracking-tight">Top Sellers</h3>
 <button className="text-[10px] font-bold text-orange-700 uppercase tracking-widest hover:text-blue-800 transition-colors">Xem tất cả</button>
 </div>
 <div className="divide-y divide-stone-100">
 {sellerData.map((seller, index) => (
 <div key={seller.name} className="flex items-center justify-between p-4 hover:bg-stone-50 transition-colors group cursor-pointer">
 <div className="flex items-center gap-3">
 ...
 {config.showSLA && (
 <div className="bg-stone-900 text-[#FAF9F5] p-6 rounded-xl shadow-sm relative overflow-hidden group">
 ...
 <h3 className="font-bold text-[#FAF9F5] text-lg flex items-center gap-2">
 ...
 <h3 className="font-bold text-stone-900 text-lg">Đơn hàng theo Giờ</h3>
 </div>
