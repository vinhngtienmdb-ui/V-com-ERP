...
const StatCard = ({ title, value, change, icon: Icon, trend, subValue, color }: any) => (
  <div className={cn("bg-white p-3 rounded-lg border border-slate-300 hover:border-slate-400 transition-all duration-300 group relative overflow-hidden flex flex-col justify-between", color)}>
    ...
    <div className="relative z-10 flex flex-col h-full">
      ...
      <div className="text-xl font-semibold text-slate-900 tracking-tight mt-auto">{value}</div>
      {subValue && (
...
export function Dashboard() {
 ...
 return (
  ...
  <div className="flex flex-col md:flex-row justify-between items-end gap-4 pb-4 border-b border-slate-200">
    <div>
      <h2 className="text-3xl font-semibold tracking-tight text-primary-900 font-sans">Chào buổi sáng, {staffInfo?.email || 'admin@vcomm.vn'}</h2>
 ...
 <span className="px-3 py-1 bg-slate-100 text-orange-800 text-[10px] rounded-lg border border-orange-200/60">Bảng điều khiển Trực tiếp</span>
 ...
 <h1 className="font-sans tracking-tight text-3xl font-semibold text-slate-900 tracking-tight">Tổng quan Hệ thống</h1>
 ...
 <div>
 <h4 className="text-sm font-semibold text-red-900 tracking-tight flex items-center gap-2">
 ...
 >
  ...
  <button className="text-[10px] text-orange-700 hover:text-blue-800 transition-colors pointer-events-auto">Xem tất cả</button>
  </div>
 ...
 >
  ...
  <div className="px-3 py-1 bg-emerald-500/20 text-emerald-400 text-[10px] rounded-full border border-emerald-500/30 flex items-center gap-1.5 pointer-events-none">
  ...
  <div className="text-[10px] text-slate-500 mb-1">Giao đúng hạn</div>
  ...
  <div className="bg-white/5 border border-white/10 rounded-lg p-4">
  <div className="text-[10px] text-slate-500 mb-1">Lỗi / Sự cố</div>
  <div className="text-2xl font-bold flex items-baseline gap-1">
  ...
  <div>
  <div className="text-[10px] text-slate-500 mb-1">Xử lý Đơn (TB)</div>
  <div className="text-2xl font-bold flex items-baseline gap-1">
  ...
  <span className="text-sm font-semibold text-[#FAF9F5]">{item.value}{item.unit}</span>
  </div>
