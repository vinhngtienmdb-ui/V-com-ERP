...
export function IPosModule() {
 ...
 <div className="flex justify-between items-center pb-3 border-b border-stone-200 text-sm">
 <span className="font-bold text-stone-500 uppercase">Đối soát hệ thống (Lệch)</span>
...
{showShiftSummary && (
 ...
 <div>
 <h2 className="text-xl font-bold text-stone-900">Báo cáo chốt ca</h2>
...
<div className="pt-6 border-t border-stone-100 flex gap-4">
 ...
 {cart.map(item => (
 <div key={item.id} className="flex justify-between gap-4 animate-in fade-in slide-in- transition-all">
 <div className="flex-1">
 <p className="text-sm font-bold text-stone-800 line-clamp-1">{item.name}</p>
 <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">x{item.quantity} • {formatCurrency(item.price)}</p>
 </div>
 ...
 <span className="text-[10px] font-black uppercase tracking-widest text-stone-400">Cần thanh toán</span>
 ...
 <div>
 <h3 className="text-2xl font-black text-stone-900 tracking-tight">Chọn phương thức</h3>
 <p className="text-xs text-stone-400 font-bold mt-1">Đảm bảo thanh toán an toàn & bảo mật</p>
 </div>
 <button 
...
{customer && (customer.points || 0) > 0 && (
 ...
 <div>
 <p className="text-sm font-black text-stone-900 uppercase tracking-tight">Thanh toán bằng Điểm Loyalty</p>
 ...
 useLoyaltyPoints ? "bg-emerald-600 border-emerald-600 text-[#FAF9F5]" : "bg-white border-stone-200 group-hover:border-emerald-500"
 )}>
 {useLoyaltyPoints ? <CheckCircle2 className="w-4 h-4" /> : <div className="w-2 h-2 bg-stone-100 rounded-full" />}
 </div>
 </div>
 )}
...
<p className="text-xs text-indigo-600 font-bold uppercase tracking-widest flex items-center gap-1.5">
 ...
 {[1, 2, 3].map(i => (
 <div key={i} className="w-6 h-6 rounded-full border-2 border-white bg-stone-200 flex items-center justify-center">
 <Building2 className="w-3 h-3 text-stone-400" />
 </div>
 ))}
 </div>
 <span className="text-[9px] font-black text-stone-400 uppercase tracking-widest">Linked with 30+ Banks</span>
 </div>
 </div>
 </motion.div>
 ) : paymentMethod === 'pos' ? (
...
>
 ...
 <select 
 className="text-sm font-bold text-stone-700 bg-transparent outline-none cursor-pointer hover:text-orange-700 transition-colors"
...
<button onClick={() => setActiveTab('promotions')} className={cn("px-5 py-2 rounded-sm text-xs font-semibold transition-all flex items-center gap-2", activeTab === 'promotions' ? "bg-white text-orange-700 shadow-sm ring-1 ring-stone-200" : "text-stone-500 hover:text-stone-700 hover:bg-stone-100")}>
 ...
 <span className="ml-1 w-5 h-5 bg-rose-500 text-[#FAF9F5] text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white">{pendingEMenuOrders.length}</span>
 )}
 </button>
 ...
 <div className="space-y-1">
 <h2 className="text-3xl font-black text-stone-900 tracking-tight">Chào buổi sáng, {user?.displayName || 'Admin'}!</h2>
 ...
 <div>
 <p className="text-sm font-bold text-stone-900">{ch.name}</p>
 <p className="text-xs font-medium text-stone-500">{ch.stats.online ? 'Online' : 'Offline'}</p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-lg font-black text-orange-700 leading-none">{ch.stats.activeDrivers}</p>
 <p className="text-[10px] font-medium text-stone-500 mt-1">Tài xế gần đây</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="flex -space-x-2">
 {[1, 2, 3].map(i => (
 <div key={i} className="w-8 h-8 rounded-full border-2 border-stone-900 bg-stone-800 flex items-center justify-center text-[10px] font-bold">
 {i}
 </div>
 ))}
 </div>
 ...
 <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest mt-1">{tip.c}</p>
 </div>
 </div>
 ))}
 </div>
...
>
 ...
 {product.stock <= 10 && (
 <div className="bg-rose-500 text-[#FAF9F5] rounded-full p-1.5 shadow-sm shadow-rose-200 animate-pulse">
 ...
 <div className="px-1">
 <h3 className="font-bold text-stone-800 leading-tight text-sm mb-1.5 group-hover:text-indigo-600 transition-colors line-clamp-2">{product.name}</h3>
 <div className="flex justify-between items-center">
 <p className="text-base font-black text-stone-900 tracking-tight">{formatCurrency(product.price)}</p>
 <span className={cn(
 ...
 <div>
 <div className="flex items-center gap-2">
 <p className="font-black text-stone-900 text-sm tracking-tight">{customer.name}</p>
 <span className="text-[8px] bg-indigo-600 text-[#FAF9F5] font-black px-1.5 py-0.5 rounded uppercase tracking-widest">{customer.tier || 'LOYAL'}</span>
 </div>
 <p className="text-[10px] text-stone-400 font-bold mt-0.5">{customer.phone} <span className="mx-1.5 opacity-30">•</span> {customer.points || 0} pts</p>
 </div>
 </div>
 <button 
 ...
 className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-stone-300 hover:text-rose-500"
 >
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 >
 <div>
 <p className="text-sm font-black text-stone-900 group-hover/item:text-indigo-600 transition-colors uppercase tracking-tight">{c.name}</p>
 <p className="text-[10px] text-stone-400 font-bold mt-0.5">{c.phone}</p>
 </div>
 <ArrowRight className="w-4 h-4 text-stone-200 group-hover/item:text-indigo-600 group-hover/item:translate-x-1 transition-all" />
 </button>
 ))}
 </div>
 )}
 ...
 <p className="font-bold text-stone-700 text-sm group-hover:text-orange-700 transition-colors">Khách hàng mới</p>
 <p className="text-xs text-stone-500 mt-1">Tích điểm • Nhận số ĐT</p>
 </div>
 </button>
 )}
 </div>
 ...
 <div>
 <h3 className="font-bold text-stone-900 text-sm">Đơn hàng hiện tại</h3>
 <p className="text-xs text-orange-700 font-bold mt-0.5">{cart.reduce((a, b) => a + b.quantity, 0)} món</p>
 </div>
 </div>
 <div className="flex gap-2">
...
{item.note && (
 ...
 <button onClick={() => removeFromCart(item.id)} className="p-2 text-stone-200 hover:text-rose-500 transition-colors">
 <Trash2 className="w-4 h-4" />
 </button>
 <p className="font-black text-stone-900 text-sm tracking-tight">{formatCurrency(item.price * item.quantity)}</p>
 </div>
 </div>
 ))}
 {cart.length > 0 && (
 ...
 <div className="w-20 h-20 bg-stone-50 rounded-full flex items-center justify-center text-stone-200 border-2 border-dashed border-stone-100">
 <ShoppingCart className="w-10 h-10" />
 </div>
 <div>
 <p className="text-xs font-bold text-stone-900 uppercase tracking-widest">Giỏ hàng trống</p>
 <p className="text-[10px] text-stone-400 mt-2 max-w-[160px] mx-auto">Vui lòng chọn sản phẩm để thanh toán</p>
 </div>
 </div>
 )}
 </div>

 <div className="p-6 bg-white border-t border-stone-100 space-y-5 rounded-b-2xl shrink-0">
 <div className="space-y-3">
 <div className="flex justify-between items-center text-stone-500">
 <span className="text-xs font-semibold uppercase tracking-wider">Tạm tính</span>
 <span className="text-sm font-bold text-stone-700">{formatCurrency(subtotal)}</span>
 </div>
 {discount > 0 && (
 ...
 <span className="flex items-center gap-2 px-2 py-1 bg-stone-50 rounded border border-stone-200"><div className="w-2.5 h-2.5 rounded-full bg-white border border-stone-300" /> Trống ({tables.filter(t => t.status === 'available').length})</span>
 ...
 <span className="flex items-center gap-2 px-2 py-1 bg-[#F2F0E9] text-orange-700 rounded border border-[#EAE7DF]"><div className="w-2.5 h-2.5 rounded-full bg-blue-400" /> Đang dọn ({tables.filter(t => t.status === 'cleaning').length})</span>
 </div>
 </div>
 ...
 "bg-[#F2F0E9] border-orange-200 text-blue-900"
 )}
 onClick={() => {
 ...
 <p className="text-[9px] font-bold text-stone-400 opacity-60 uppercase tracking-widest">{table.zone} • {table.capacity} chỗ</p>
 </div>
 ...
 <div>
 <h2 className="text-xl font-black text-stone-900 tracking-tight">Hệ thống Quản trị & Phân quyền</h2>
 ...
 <p className="text-xl font-black text-stone-900 tracking-tight">{stat.value}</p>
 </div>
 ))}
 </div>
 ...
 <span className="text-xs font-bold text-stone-600">{p.name}</span>
 </div>
 <span className="text-xs font-black text-stone-900">{p.value}%</span>
 </div>
 ))}
 </div>
 </div>
 ...
 platform.status === 'Đã kết nối' ? "bg-emerald-50 text-emerald-600" : "bg-stone-50 text-stone-400"
 )}>
 {platform.status}
 </span>
 <div className="mt-4 flex items-center gap-2">
 <div className={cn("w-2 h-2 rounded-full", platform.status === 'Đã kết nối' ? "bg-emerald-500 animate-pulse" : "bg-stone-200")} />
 <span className="text-[10px] font-bold text-stone-400">API: Up</span>
 </div>
 </div>
 </div>
 ...
 <div>
 <h3 className="font-bold text-stone-800">Danh sách nhân sự ({staffList.length})</h3>
 <p className="text-xs text-stone-500 font-medium">Quản lý tài khoản truy cập và vai trò của nhân viên tại chi nhánh này.</p>
 </div>
 <div className="flex items-center gap-3">
 <button 
 ...
 <h4 className="font-bold text-stone-900 group-hover:text-indigo-600 transition-colors">{staff.fullName}</h4>
 <p className="text-xs text-stone-500 font-medium mb-1">{staff.email}</p>
 <p className="text-[10px] text-stone-300 font-mono font-bold">{staff.id}</p>
 
 <div className="mt-6 pt-6 border-t border-stone-50 flex justify-between items-center">
 <div className="flex items-center gap-2">
 <div className={cn("w-2 h-2 rounded-full", staff.status === 'active' ? "bg-emerald-500" : "bg-stone-300")} />
 ...
 <div className="space-y-2">
 <h3 className="font-bold text-stone-800">Cấu hình Chi nhánh</h3>
 <p className="text-xs text-stone-500 font-medium">Thiết lập các quy tắc vận hành cho chi nhánh <span className="font-bold text-indigo-600">{activeStore?.name}</span>.</p>
 </div>
 ...
 <p className="text-xs text-stone-500">Yêu cầu nhân viên khai báo két tiền trước khi bắt đầu bán hàng.</p>
 </div>
 <button 
 ...
 <div className="flex justify-between items-center">
 <div className="space-y-0.5">
 <p className="text-sm font-bold text-stone-900">Tự động in hóa đơn</p>
 <p className="text-xs text-stone-500">In phiếu thu ngay sau khi hoàn tất thanh toán.</p>
 </div>
 <button 
 ...
 <div className="flex justify-between items-center">
 <div className="space-y-0.5">
 <p className="text-sm font-bold text-stone-900">Cho phép trả hàng</p>
 <p className="text-xs text-stone-500">Kích hoạt tính năng hoàn tiền cho các hóa đơn đã thanh toán.</p>
 </div>
 <button 
 ...
 <p className="text-xs text-stone-400 font-bold uppercase tracking-widest mt-1">Đơn đặt hàng từ ứng dụng ngoài</p>
 </div>
 </div>
 ...
 <div className="text-center">
 <h3 className="text-lg font-black text-stone-900 mb-2">Phát hiện Sai lệch Thông tin</h3>
 <p className="text-sm text-stone-600 leading-relaxed">{discrepancyPrompt.message}</p>
 </div>
 ...
 <p className="text-sm font-bold text-stone-400">Không có đơn hàng mới nào</p>
 </div>
 ) : (
 ...
 order.platform === 'ShopeeFood' ? 'bg-orange-500' : 'bg-stone-400'
 )} />
 <div className="flex justify-between items-start mb-4">
 <div>
 <span className={cn(
 ...
 order.platform === 'ShopeeFood' ? 'bg-orange-50 text-orange-700' : 'bg-stone-50 text-stone-700'
 )}>{order.platform}</span>
 <h3 className="font-bold text-stone-900 mt-2">{order.customerName}</h3>
 <p className="text-[10px] text-stone-500 font-mono font-bold mt-0.5">{order.targetPhone}</p>
 </div>
 <div className="text-right">
 <p className="text-sm font-black text-indigo-600">{formatCurrency(order.total)}</p>
 <p className="text-[10px] text-stone-400 font-bold mt-1">{order.items.length} món</p>
 </div>
 </div>
 ...
 <span className="text-[10px] font-bold text-stone-400">Yêu cầu từ khách tại bàn</span>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 ...
 <div>
 <p className="font-black text-stone-900">BÀN {order.tableId}</p>
 <p className="text-[10px] text-stone-500 font-bold uppercase tracking-widest">{order.items?.length || 0} món • {formatCurrency(order.total)}</p>
 </div>
 </div>
 <span className="text-[10px] font-bold text-stone-400">{order.time || 'Vừa xong'}</span>
 </div>
 ...
 <div className="text-right">
 <p className="font-bold text-stone-900 text-lg">{formatCurrency(tx.total)}</p>
 ...
 <h1 className="font-serif tracking-tight text-sm font-bold uppercase">VComm ERP iPOS System</h1>
 ...
 <div className="space-y-3">
 <label className="text-[10px] font-black uppercase tracking-widest text-stone-400 ml-1">Vai trò & Phân quyền</label>
 ...
 <role.icon className={cn("w-6 h-6 mb-1", staffForm.role === role.id ? "text-indigo-600" : "text-stone-300")} />
 <p className={cn("text-xs font-black", staffForm.role === role.id ? "text-indigo-600" : "text-stone-600")}>{role.name}</p>
 <p className="text-[9px] font-medium text-stone-400">{role.desc}</p>
 </button>
 ))}
 </div>
 </div>
 ...
 <div>
 <h3 className="text-lg font-black text-stone-900 tracking-tight">Chi tiết Hóa đơn</h3>
 ...
 <p className="text-xs font-black text-stone-900 uppercase tracking-widest">{activeStore?.name}</p>
 <p className="text-[10px] text-stone-500 font-medium">{activeStore?.address}</p>
 <p className="text-[10px] text-stone-500 font-medium">SĐT: 1900 1234</p>
 </div>
 ...
 <div>
 <p className="text-xs font-black text-stone-900">Sản phẩm mẫu {i+1}</p>
 <p className="text-[10px] text-stone-400 font-medium">1 x {formatCurrency(selectedTxForDetail.total / 2)}</p>
 </div>
 <span className="text-xs font-black text-stone-900">{formatCurrency(selectedTxForDetail.total / 2)}</span>
 </div>
 ))}
 </div>
 ...
 <span className="text-sm font-black text-stone-900 uppercase tracking-widest">Tổng cộng</span>
 ...
 <p className="text-[10px] text-stone-400 font-medium italic">Vui lòng giữ lại hóa đơn để được hỗ trợ bảo hành & đổi trả trong vòng 7 ngày.</p>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-xl font-black text-stone-900 tracking-tight">QR E-Menu</h3>
 ...
 <div className="space-y-4">
 <p className="text-[10px] text-stone-400 italic font-medium leading-relaxed uppercase tracking-tighter">Khách hàng quét mã này để truy cập thực đơn trực tuyến và đặt món trực tiếp tại bàn.</p>
 <button 
 onClick={() => {
