        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex gap-2">
               <div className="relative">
                  ...
                  <input type="text" placeholder="Tra mã đơn, SĐT khách..." className="pl-9 pr-4 py-2 bg-slate-100 border border-slate-300 rounded-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary-100 w-64" />
               </div>
               <button className="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-sm text-xs font-bold shadow-sm hover:bg-slate-50 flex items-center gap-2">
        ...
        <div className="flex gap-4">
            {[
              ...
              { id: 'history', label: 'Lịch sử Sửa/Xóa' }
               ...
               <button key={t.id} onClick={() => setTab(t.id as any)} className={cn("pb-2 text-sm font-bold border-b-2 transition-colors", tab === t.id ? "border-primary-600 text-primary-600" : "border-transparent text-slate-500 hover:text-slate-700")}>
                  {t.label}
               </button>
            ))}
        </div>
