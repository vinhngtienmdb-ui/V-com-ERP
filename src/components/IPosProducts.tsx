      ...
      <div className="bg-white border-b border-stone-200 px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
           <div className="relative">
              ...
              <input type="text" placeholder="Tìm SKU, tên..." className="pl-9 pr-4 py-2 bg-stone-100 border border-stone-200 rounded-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary-100 w-64" />
           </div>
         ...
         <div className="bg-white border border-stone-200 rounded-sm shadow-sm md:flex min-h-[500px]">
             ...
             <div className="w-full md:w-48 xl:w-56 border-r border-stone-200 p-4 space-y-1">
                 ...
                 {['Tất cả Sản phẩm', 'Lô Sản phẩm / HSD', 'Thương hiệu', 'Thời gian Lưu kho', 'Tồn kho thấp', 'Danh mục', 'Thuộc tính', 'Nhà cung cấp'].map((item, i) => (
                    <button key={item} className={cn("w-full text-left px-3 py-2 text-xs font-bold rounded-sm transition-colors", i===0 ? "bg-primary-50 text-primary-700" : "text-stone-600 hover:bg-stone-50")}>
                        {item}
                    </button>
                 ))}
             </div>
