    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Cài đặt iPOS & Phân quyền</h1>
            <p className="text-sm text-slate-500 mt-1">Thiết lập vai trò nâng cao và giới hạn quyền truy cập từng module iPOS.</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={() => navigate('/ipos')} className="px-4 py-2 border border-slate-200 text-slate-700 bg-white rounded-lg text-sm font-semibold hover:bg-slate-50 shadow-sm">
            Quay lại iPOS
          </button>
          <button onClick={handleSave} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 shadow-sm flex items-center gap-2">
            ...
            <div className="space-y-2">
              {roles.map(role => (
                ...
                >
                  <p className={cn(
                    "font-bold text-sm",
                    activeRole === role.id ? "text-indigo-700" : "text-slate-700"
                  )}>{role.name}</p>
                  <p className={cn(
                    "text-[11px] mt-1 line-clamp-2",
                    activeRole === role.id ? "text-indigo-500/80" : "text-slate-500"
                  )}>{role.description}</p>
                </button>
              ))}
            </div>
        ...
        <div className="flex-1 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">Gạt công tắc để cấp hoặc thu hồi quyền truy cập tính năng.</p>
            </div>
            {activeRole === 'admin' && (
          ...
          <div className="p-0">
            {IPOS_MODULES.map((module, mIdx) => (
              <div key={module.id} className={cn(
                "p-6",
                mIdx !== IPOS_MODULES.length - 1 ? "border-b border-slate-100" : ""
              )}>
                    ...
                    return (
                      <label 
                        ...
                        className={cn(
                          ...
                          isGranted ? "bg-slate-50 border-emerald-200" : "bg-white border-slate-200 hover:border-slate-300",
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">Mã quyền: {permKey}</p>
                        </div>
                      </label>
                    );
                  })}
