...
export function LoginPage() {
 const { login, isMfaRequired, mfaUserEmail, verifyMfaCode, cancelMfa } = useAuth();
 ...
 <div className="min-h-screen bg-[#F9FAFB] flex flex-col md:flex-row font-sans">
  ...
  {/* Right Pane - Login Form / MFA Verification */}
  <div className="flex-1 flex items-center justify-center p-6 md:p-6">
  <div className="max-w-sm w-full space-y-10 animate-in fade-in duration-300">
    ...
    <>
      <div className="space-y-2">
        ...
        <p className="text-[#6B7280] text-sm font-medium">
          Mã xác thực đã được yêu cầu cho tài khoản <span className="text-slate-900 font-bold">{mfaUserEmail}</span>.
        </p>
      </div>
      ...
      <form onSubmit={handleMfaSubmit} className="space-y-6">
        ...
        {error && (
          <div className="bg-red-50 border border-red-100 rounded-lg p-4 flex items-start gap-3">
            ...
            <div className="text-xs font-medium text-red-600 leading-relaxed">
              {error} <br />
              <span className="opacity-70">Vui lòng kiểm tra lại mã số từ Google/Microsoft Authenticator.</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3">
          ...
          <button 
            type="button"
            onClick={cancelMfa}
            className="w-full py-3 bg-white border border-slate-300 text-slate-700 rounded-lg font-bold text-xs uppercase tracking-wider hover:bg-slate-50 transition-all text-center"
          >
            Quay lại Đăng nhập
          </button>
        </div>
