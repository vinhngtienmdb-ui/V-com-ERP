...
import { chatThreadsRepo, type ChatThreadInput } from '../services/repositories';
...
/** Map ChatThreadInput → ChatThread legacy UI shape */
function adaptThread(t: ChatThreadInput): ChatThread {
 return {
   id: t.id,
   channel: (t.channel === 'webchat' ? 'web' : t.channel) as ChatChannel,
...
export function OmniChat() {
 ...
 useEffect(() => {
   const unsub = chatThreadsRepo.subscribe([orderBy('lastMessageAt', 'desc')], (items) => {
     setDbThreads(items.map(adaptThread));
   });
   return () => unsub();
 }, []);

 // Fallback MOCK khi DB rỗng
 const threadsToShow = dbThreads.length > 0 ? dbThreads : MOCK_THREADS;
 ...
 const activeThread = threadsToShow.find(t => t.id === activeThreadId);
 ...
 className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-600/20 transition-all"
 />
