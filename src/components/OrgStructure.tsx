...
export function OrgStructure() {
 ...
 <div>
 <h1 className="font-serif tracking-tight text-2xl font-semibold text-[#111827]">Cơ cấu Tổ chức</h1>
 <p className="text-sm text-[#6B7280]">Quản lý sơ đồ bộ máy phòng ban, chức danh và cấp bậc trong hệ thống.</p>
 </div>
 <button className="bg-[#2563EB] text-[#FAF9F5] px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-stone-800">
 ...
 <section.icon className="w-5 h-5 text-orange-700" /> {section.title}
 </h3>
 <div className="space-y-3">
 {section.data.map((item: any, i) => (
 <div key={i} className="bg-stone-50 p-4 rounded-lg border border-stone-100 flex justify-between items-center hover:shadow-sm transition-all">
