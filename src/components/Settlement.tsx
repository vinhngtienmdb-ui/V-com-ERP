...
export function SettlementManagement() {
 ...
 <h1 className="font-sans tracking-tight text-xl font-bold text-slate-900">Đối soát & Hóa đơn Điện tử</h1>
 <p className="text-sm text-slate-500 mt-1">Đối soát dòng tiền Seller, xử lý yêu cầu rút tiền và tự động xuất hóa đơn GTGT.</p>
 </div>
 ...
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Số dư Ví Sàn (Tất cả)</p>
 <div className="text-xl font-bold text-slate-900">{formatCurrency(15450000000)}</div>
 ...
 <div className="bg-white p-5 rounded-2xl border border-slate-300 shadow-sm">
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Đang chờ giải ngân (Payout)</p>
 <div className="text-2xl font-bold text-blue-600">{formatCurrency(2450000000)}</div>
 <p className="text-[10px] text-slate-500 mt-1">Sẽ tự động chuyển sau 3 ngày</p>
 </div>
 <div className="bg-white p-5 rounded-2xl border border-slate-300 shadow-sm">
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Lệnh rút tiền chờ duyệt</p>
 <div className="text-2xl font-bold text-[#F59E0B]">42</div>
 <p className="text-[10px] text-slate-500 mt-1">Ưu tiên: Nhà bán hàng (35)</p>
 </div>
 <div className="bg-white p-5 rounded-2xl border border-slate-300 shadow-sm">
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Doanh thu hoa hồng (Margin)</p>
 <div className="text-2xl font-bold text-[#10B981]">{formatCurrency(845000000)}</div>
 <p className="text-[10px] text-slate-500 mt-1">Tháng: 03/2024</p>
 </div>
 </DraggableGrid>
 ...
 <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center bg-slate-50">
 ...
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Nhà bán hàng (Seller)</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap">Kỳ đối soát</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap text-right">Tổng Doanh số</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-32 whitespace-nowrap text-right">Phí sàn/Ship</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap text-right">Tiền về (Payout)</th>
 ...
 <tr className="bg-slate-50 border-b border-slate-100">
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Người dùng / Đối tượng</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Thông tin Ngân hàng</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-32 whitespace-nowrap text-right">Số tiền rút</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap text-right">Thời gian yêu cầu</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-32 whitespace-nowrap text-center">Trạng thái duyệt</th>
 </tr>
 )}
 {activeTab === 'cod' && (
 <tr className="bg-slate-50 border-b border-slate-100">
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Đơn vị Vận chuyển</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap">Kỳ đối soát / Số ĐH</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-32 whitespace-nowrap text-right">Tổng Cước phí</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap text-right">COD Hệ thống ghi nhận</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-36 whitespace-nowrap text-right">COD Thực chuyển</th>
 <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest w-32 whitespace-nowrap text-center">Trạng thái Kế toán</th>
 </tr>
 )}
 </thead>
 <tbody className="divide-y divide-slate-100">
 {activeTab === 'settlement' && MOCK_SETTLEMENTS.map((stl) => (
 <tr key={stl.id} className="hover:bg-slate-50 group transition-colors">
 <td className="px-6 py-4">
 <p className="text-sm font-bold text-slate-900">{stl.sellerName}</p>
 <p className="text-[10px] text-slate-500 font-mono uppercase tracking-tight">{stl.sellerId}</p>
 </td>
 ...
 <tr key={wdr.id} className="hover:bg-slate-50 group transition-colors">
 <td className="px-6 py-4">
 <p className="text-sm font-bold text-slate-900">{wdr.userName}</p>
 <span className="text-[10px] text-slate-500 uppercase font-semibold bg-slate-100 px-1.5 py-0.5 rounded">{wdr.userType}</span>
 </td>
 <td className="px-6 py-4 max-w-[220px]">
 <p className="text-xs font-bold text-slate-900 truncate">{wdr.bankAccount.bankName}</p>
 <p className="text-[10px] font-mono text-slate-500 truncate">{wdr.bankAccount.accountNo} - {wdr.bankAccount.accountName}</p>
 </td>
 <td className="px-6 py-4 w-32 whitespace-nowrap text-right font-bold text-slate-900">{formatCurrency(wdr.amount)}</td>
 <td className="px-6 py-4 w-36 whitespace-nowrap text-right text-[10px] text-[#9CA3AF]">{wdr.requestDate}</td>
 <td className="px-6 py-4 text-right">
 {wdr.status === 'pending' ? (
 ...
 <button className="px-3 py-1.5 border border-slate-300 text-slate-500 text-[10px] font-bold rounded-md">Từ chối</button>
 </div>
 ) : (
 ...
 <tr key={cod.id} className="hover:bg-slate-50 group transition-colors">
 <td className="px-6 py-4">
 <p className="text-sm font-bold text-slate-900">{cod.carrier}</p>
 <p className="text-[10px] text-slate-500 font-mono uppercase tracking-tight">{cod.id}</p>
 </td>
 <td className="px-6 py-4">
 <p className="text-xs font-bold text-[#4B5563]">{cod.period}</p>
 <p className="text-[10px] text-slate-500">Tổng {cod.totalOrders} đơn</p>
 </td>
 ...
 <div className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg p-8 overflow-hidden relative">
 ...
 <div className="bg-slate-800/50 px-4 py-3 rounded-2xl border border-slate-700 flex flex-col">
