...
export function Sidebar() {
  ...
  return (
  <aside className="w-[280px] bg-white border-r border-slate-200 flex flex-col h-full py-6">
  ...
  {group.items.map((item: any) => {
    ...
    return (
      ...
      className={cn(
      "w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all duration-200 group relative overflow-hidden",
      isActive 
      ? "bg-primary-50/80 text-primary-600 font-bold" 
      : "text-[#4B5563] hover:bg-slate-50 hover:text-[#111827]"
      )}
      >
      <item.icon className={cn(
      "w-5 h-5 transition-transform duration-200 ",
      isActive ? "text-primary-600" : "text-[#9CA3AF]"
      )} />
