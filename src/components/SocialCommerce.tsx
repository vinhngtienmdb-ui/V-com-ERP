...
export function SocialCommerce() {
 ...
 <p className="text-[10px] text-stone-400 mt-1">Tỷ lệ mua hàng từ Social Feed</p>
 </div>
 </div>
 ...
 <div className="w-10 h-10 rounded-full bg-[#E5E7EB] flex items-center justify-center font-bold text-stone-400">
 {post.authorName[0]}
 </div>
 <div>
 ...
 <button className="p-2 hover:bg-stone-50 rounded-lg"><MoreHorizontal className="w-4 h-4 text-stone-400" /></button>
 </div>
 <p className="text-sm text-[#374151] leading-relaxed">{post.content}</p>
 <div className="h-48 bg-stone-100 rounded-lg flex items-center justify-center relative overflow-hidden group">
 <ImageIcon className="w-8 h-8 text-stone-300 group-hover:scale-110 transition-transform" />
 ...
 <button className="flex items-center gap-1.5 text-xs font-bold text-stone-500 hover:text-red-500">
 <Heart className="w-4 h-4" /> {post.likes.toLocaleString()}
 </button>
 <button className="flex items-center gap-1.5 text-xs font-bold text-stone-500 hover:text-[#2563EB]">
 <MessageSquare className="w-4 h-4" /> {post.comments.toLocaleString()}
 </button>
 <button className="flex items-center gap-1.5 text-xs font-bold text-stone-500 hover:text-emerald-500">
 <Share2 className="w-4 h-4" /> Chia sẻ
 </button>
 </div>
 <button className="bg-[#F9FAFB] px-4 py-2 rounded-lg text-[10px] font-bold text-stone-600 hover:bg-stone-100 transition-all uppercase tracking-widest">Ghim sản phẩm trong bài</button>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-lg bg-stone-50 flex items-center justify-center text-stone-400 group-hover:text-[#2563EB] transition-colors">
 <Hash className="w-4 h-4" />
 </div>
 <div>
 <p className="text-xs font-bold text-[#111827]">#{h.tag}</p>
 <p className="text-[10px] text-stone-500">{h.posts} bài viết</p>
 </div>
 </div>
 {h.trend === 'up' ? <TrendingUp className="w-4 h-4 text-emerald-500" /> : <TrendingUp className="w-4 h-4 text-stone-300 rotate-180" />}
 </div>
 ))}
 </div>
 </div>
 ...
 <button className="w-full py-3 bg-stone-900 text-[#FAF9F5] font-bold rounded-lg text-xs hover:bg-stone-800 transition-all uppercase tracking-widest shadow-sm shadow-stone-900/5">Cấu hình AI Vision</button>
 </div>
