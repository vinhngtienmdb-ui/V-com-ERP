...
/** Trả về chuỗi địa chỉ đầy đủ: "Số nhà, Phường/Xã, Tỉnh/TP"*/ export function formatAddress(a: VietnamAddress): string { return [a.street, a.wardName, a.provinceName].filter(Boolean).join(', '); } /* ───────────────────────────────────────────────────────────────────────── Cascade selector — 2 cấp: Tỉnh/Thành phố → Phường/Xã ───────────────────────────────────────────────────────────────────────── */ interface SelectorProps { value: VietnamAddress; onChange: (v: VietnamAddress) => void; className?: string; disabled?: boolean; showStreet?: boolean; streetPlaceholder?: string; compact?: boolean; } export function VietnamAddressSelector({ value, onChange, className, disabled = false, showStreet = true, streetPlaceholder = 'Số nhà, tên đường...', compact = false, }: SelectorProps) { const [provinces, setProvinces] = useState<Province[]>([]); const [wards, setWards] = useState<Ward[]>([]); const [loadingP, setLoadingP] = useState(false); const [loadingW, setLoadingW] = useState(false); const [errorP, setErrorP] = useState(false); const [errorW, setErrorW] = useState(false); /* Tải danh sách tỉnh/thành khi mount */ const loadProvinces = useCallback(() => { setLoadingP(true); setErrorP(false); fetch(`${API_BASE}/?depth=1`) .then(r => { if (!r.ok) throw new Error(); return r.json(); }) .then((data: Province[]) => setProvinces(data)) .catch(() => setErrorP(true)) .finally(() => setLoadingP(false)); }, []); useEffect(() => { loadProvinces(); }, [loadProvinces]); /* Tải phường/xã khi chọn tỉnh (depth=2 → trực tiếp trả về wards) */ useEffect(() => { if (!value.provinceCode) { setWards([]); return; } setLoadingW(true); setErrorW(false); fetch(`${API_BASE}/p/${value.provinceCode}?depth=2`) .then(r => { if (!r.ok) throw new Error(); return r.json(); }) .then((data: Province & { wards?: Ward[] }) => setWards(data.wards ?? [])) .catch(() => { setWards([]); setErrorW(true); }) .finally(() => setLoadingW(false)); }, [value.provinceCode]); const handleProvince = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => { const code = e.target.value ? Number(e.target.value) : null; const prov = provinces.find(p => p.code === code); onChange({ ...value, provinceCode: code, provinceName: prov?.name ?? '', wardCode: null, wardName: '' }); }, [provinces, value, onChange]); const handleWard = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => { const code = e.target.value ? Number(e.target.value) : null; const ward = wards.find(w => w.code === code); onChange({ ...value, wardCode: code, wardName: ward?.name ?? '' }); }, [wards, value, onChange]); const inputCls = cn( 'w-full border border-slate-200 bg-white text-slate-800', 'focus:outline-none focus:border-blue-500 transition-colors appearance-none', compact ? 'px-2.5 py-1.5 text-[13px]' : 'px-3 py-2 text-sm', disabled && 'opacity-50 cursor-not-allowed bg-slate-50', ); const labelCls = 'block font-mono text-[10px] text-slate-500 mb-1'; return ( <div className={cn('space-y-2.5', className)}> {/* ── Tỉnh / Thành phố ── */} <div> <label className={labelCls}>Tỉnh / Thành phố</label> <div className="relative">
          <select
