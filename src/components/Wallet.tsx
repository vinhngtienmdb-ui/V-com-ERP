...
export function WalletHub() {
 ...
 <h3 className="text-xl font-black text-stone-900 tracking-tight italic uppercase">
 ...
 <label className="text-[10px] font-black uppercase tracking-widest text-stone-400 ml-1">Số tiền (VNĐ)</label>
 <input 
 type="number" 
 className="w-full bg-stone-50 border border-stone-200 rounded-lg px-6 py-4 text-2xl font-black text-stone-900 focus:ring-4 focus:ring-blue-50 focus:border-stone-900 transition-all outline-none"
 placeholder="0"
 ...
 selectedBank.id === bank.id ? "border-stone-900 bg-[#F2F0E9]" : "border-stone-100 bg-white hover:border-stone-200"
 )}
 >
 <div className="flex items-center gap-3">
 <div className={cn(
 "w-10 h-10 rounded-lg flex items-center justify-center text-[#FAF9F5]",
 selectedBank.id === bank.id ? "bg-stone-900" : "bg-stone-200"
 )}>
 <Landmark className="w-5 h-5" />
 </div>
 <div className="text-left">
 <p className={cn("text-sm font-bold", selectedBank.id === bank.id ? "text-blue-900" : "text-stone-700")}>{bank.bankName}</p>
 <p className="text-[10px] text-stone-400 font-mono italic">{bank.accountNumber}</p>
 </div>
 </div>
 ...
 className="flex-[2] py-4 bg-stone-900 text-[#FAF9F5] rounded-lg font-black text-xs uppercase tracking-[0.2em] shadow-sm shadow-blue-200 hover:bg-stone-800 active:scale-95 transition-all"
 >
 Xác nhận giao dịch
 </button>
 </div>
 ...
 {/* Main Wallet Card */}
 <div className="lg:col-span-2 relative h-[240px] rounded-lg bg-white p-8 text-[#FAF9F5] shadow-sm shadow-stone-900/5 overflow-hidden group">
 ...
 className="bg-white text-orange-700 px-6 py-2.5 rounded-lg font-bold text-sm shadow-sm shadow-blue-900/10 hover:bg-[#F2F0E9] transition-all"
 >
 Nạp ví
 </button>
 <button 
 ...
 <div>
 <p className="text-[10px] font-bold text-stone-400 uppercase">Dự báo tăng trưởng</p>
 <p className="text-lg font-black text-stone-900">+24.5%</p>
 </div>
 <button className="p-2 hover:bg-stone-50 rounded-lg transition-all text-orange-700">
 ...
 <div className="bg-white rounded-lg border border-stone-200 shadow-sm overflow-hidden p-2 min-h-[600px]">
 <div className="flex border-b border-stone-100 bg-stone-50/50 p-1.5 overflow-x-auto scrollbar-hide">
 {[
 ...
 activeTab === tab.id ? "bg-white text-orange-700 shadow-sm" : "text-stone-500 hover:text-stone-700"
 )}
 >
 ...
 className="space-y-6"
 >
 <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center bg-stone-50 p-4 rounded-lg">
 ...
 className="w-full bg-white border border-stone-200 rounded-lg pl-12 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-600 outline-none"
 />
 </div>
 ...
 className="px-4 py-2 bg-white border border-stone-200 rounded-lg text-xs font-bold text-stone-600 focus:outline-none focus:border-stone-900"
 />
 <select 
 value={filterType}
 onChange={(e) => setFilterType(e.target.value)}
 className="px-3 py-2 bg-white border border-stone-200 rounded-lg text-xs font-bold text-stone-600 focus:outline-none focus:border-stone-900"
 >
 ...
 <button className="px-4 py-2 bg-[#111827] text-[#FAF9F5] rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-stone-800 transition-all ml-auto xl:ml-0">
 ...
 <th className="px-6 py-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest">Mã Giao dịch</th>
 <th className="px-6 py-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest">Loại giao dịch</th>
 <th className="px-6 py-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest text-right">Số tiền (VNĐ)</th>
 <th className="px-6 py-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest">Gateway</th>
 ...
 <tr>
 <td colSpan={6} className="py-8 text-center text-sm text-stone-500 font-medium">Không tìm thấy giao dịch nào phù hợp.</td>
 </tr>
 ) : filteredTransactions.map(txn => (
 <tr key={txn.id} className="hover:bg-stone-50 transition-all group">
 <td className="px-6 py-4">
 <div className="flex items-center gap-2">
 <div className={cn("w-1.5 h-1.5 rounded-full", txn.type === 'deposit' ? "bg-emerald-500" : "bg-stone-800")} />
 <span className="text-xs font-mono font-bold text-stone-900 group-hover:text-orange-700">{txn.id}</span>
 </div>
 </td>
 <td className="px-6 py-4">
 <span className={cn(
 ...
 txn.type === 'payout' ? "bg-[#F2F0E9] text-orange-700" : "bg-stone-100 text-stone-600"
 )}>
 {txn.type}
 </span>
 </td>
 ...
 txn.type === 'deposit' ? "text-emerald-600" : "text-stone-900"
 )}>
 {txn.type === 'deposit' ? '+' : '-'}{formatCurrency(txn.amount)}
 </td>
 <td className="px-6 py-4">
 <div className="flex items-center gap-2">
 <div className="w-6 h-6 bg-stone-100 rounded-md flex items-center justify-center">
 <Smartphone className="w-3.5 h-3.5 text-stone-400" />
 </div>
 <span className="text-xs font-bold text-stone-500 uppercase">{txn.gateway}</span>
 </div>
 </td>
 <td className="px-6 py-4">
 ...
 <td className="px-6 py-4 text-[10px] font-bold text-stone-400 uppercase tracking-tighter whitespace-nowrap">{txn.timestamp}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 {MOCK_BANK_ACCOUNTS.map(bank => (
 <div key={bank.id} className="bg-white border border-stone-100 rounded-lg p-8 shadow-sm relative overflow-hidden group">
 ...
 <div className="flex items-center gap-3">
 <div className="w-12 h-12 bg-stone-900 rounded-lg flex items-center justify-center text-[#FAF9F5] shadow-sm">
 <Building2 className="w-6 h-6" />
 </div>
 <div>
 <h4 className="font-black text-stone-900 leading-none">{bank.bankName}</h4>
 <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mt-1">{bank.type} account</p>
 </div>
 </div>
 <div>
 <p className="text-[10px] font-bold text-stone-400 uppercase">Account Number</p>
 <p className="text-xl font-mono font-bold text-stone-700 tracking-wider mt-1">{bank.accountNumber}</p>
 </div>
 <div>
 <p className="text-[10px] font-bold text-stone-400 uppercase">Account Holder</p>
 <p className="text-sm font-black text-stone-900 tracking-tight uppercase italic">{bank.accountName}</p>
 </div>
 </div>
 <div className="text-right">
 {bank.isDefault && (
 ...
 <p className="text-[10px] font-bold text-stone-400 uppercase">Balance</p>
 <p className="text-2xl font-black text-stone-900">{formatCurrency(bank.balance)}</p>
 </div>
 </div>

 <div className="mt-8 pt-6 border-t border-stone-50 flex justify-between items-center relative z-10">
 <div className="flex gap-2">
 <button className="p-2 bg-stone-50 hover:bg-stone-100 rounded-lg transition-all text-stone-400 hover:text-orange-700">
 ...
 <button className="px-4 py-2 bg-stone-900 text-[#FAF9F5] text-xs font-black uppercase rounded-lg shadow-sm shadow-blue-200 hover:bg-stone-800 transition-all flex items-center gap-2">
 ...
 <div className="bg-stone-50 border-2 border-dashed border-stone-200 rounded-lg p-8 flex flex-col items-center justify-center text-center space-y-4 hover:bg-stone-100 transition-all cursor-pointer group">
 ...
 <div>
 <h4 className="font-bold text-stone-900 text-lg">Link New Bank Account</h4>
 <p className="text-xs text-stone-500 max-w-[200px] mx-auto mt-1">Connect your corporate bank account for instant payouts.</p>
 </div>
 </div>
 </div>

 <div className="bg-white rounded-lg p-8 text-[#FAF9F5] flex flex-col md:flex-row justify-between items-center gap-8 shadow-sm shadow-blue-200 overflow-hidden relative">
 ...
 <button className="relative z-10 px-8 py-5 bg-white text-orange-700 rounded-lg font-black text-sm uppercase tracking-widest shadow-sm hover:bg-[#F2F0E9] transition-all active:scale-95 whitespace-nowrap">
 ...
 className="space-y-8"
 >
 <div className="bg-stone-900 rounded-lg p-8 text-[#FAF9F5] relative overflow-hidden flex flex-col md:flex-row gap-8 items-center">
 <div className="relative z-10 space-y-4 max-w-md">
 <div className="flex items-center gap-2 px-3 py-1 bg-stone-800/20 rounded-full w-fit">
 ...
 <div className="text-center">
 <p className="text-[10px] font-bold text-stone-500 uppercase">Protection Level</p>
 ...
 i === 1 ? "bg-stone-900 border-stone-900 shadow-[0_0_20px_rgba(37,99,235,0.4)]" : "border-white/10 bg-white/5"
 )}>
 ...
 <div className="absolute top-0 left-0 w-2/3 h-full bg-stone-800 shadow-[0_0_10px_rgba(59,130,246,0.8)]" />
 </div>
 </div>
 </div>
 ...
 <tr key={escrow.orderId} className="hover:bg-stone-50 transition-all">
 <td className="px-6 py-4 text-sm font-bold text-stone-900 group">{escrow.orderId}</td>
 ...
 <td className="px-6 py-4 text-right text-[10px] font-black text-stone-400">{escrow.autoReleaseAt}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="lg:col-span-3 border-l-4 border-stone-900 bg-[#F2F0E9]/50 p-6 rounded-lg mb-4 flex items-center justify-between">
 <div className="flex items-center gap-4">
 <div className="w-12 h-12 bg-white rounded-lg shadow-sm flex items-center justify-center">
 <Building2 className="w-6 h-6 text-orange-700" />
 </div>
 <div>
 <h4 className="font-bold text-stone-900">SePay Bank Hub Connection</h4>
 <p className="text-xs text-stone-500">Đang đồng bộ hóa dữ liệu từ 3 tài khoản ngân hàng liên kết.</p>
 </div>
 </div>
 <div className="flex items-center gap-6">
 <div className="text-right">
 <p className="text-[10px] font-bold text-stone-400 uppercase">Trạng thái API</p>
 ...
 onClick={syncBankHub}
 disabled={isSyncing}
 className="px-4 py-2 bg-white border border-stone-200 rounded-lg text-xs font-bold text-orange-700 hover:bg-white shadow-sm transition-all flex items-center gap-2"
 >
 ...
 {gateways.map(gw => (
 <div key={gw.id} className="bg-white border border-stone-100 rounded-lg p-6 shadow-sm hover:shadow-sm transition-all relative group overflow-hidden">
 {gw.isPreferred && (
 <div className="absolute top-0 right-0 p-4">
 <div className="px-2 py-1 bg-stone-900 text-[#FAF9F5] text-[8px] font-black uppercase rounded-lg shadow-sm">Preferred</div>
 </div>
 )}
 ...
 gw.provider === 'vnpay' ? "bg-stone-800" :
 gw.provider === 'momo' ? "bg-pink-600" :
 gw.provider === 'zalopay' ? "bg-stone-900 text-[#FAF9F5]" : "bg-stone-900"
 )}>
 ...
 gw.status === 'maintenance' ? "bg-amber-50 text-amber-600" : "bg-stone-50 text-stone-400"
 )}>
 ...
 <div className="flex flex-1 items-center gap-2 text-[10px] font-bold text-stone-400">
 <Wifi className="w-3 h-3 text-emerald-500" /> API Connected
 </div>
 {!gw.isPreferred && (
 <button 
 onClick={() => setPreferredGateway(gw.id)}
 className="px-2 py-1 bg-stone-100 hover:bg-stone-200 text-stone-600 text-[10px] font-bold uppercase rounded transition-all"
 >
 Đặt mặc định
 </button>
 )}
 ...
 <div className="bg-stone-50 border-2 border-dashed border-stone-200 rounded-lg p-6 flex flex-col items-center justify-center text-center space-y-3 cursor-pointer hover:bg-stone-100 transition-all">
 <div className="p-3 bg-white rounded-full shadow-sm">
 <Plus className="w-6 h-6 text-stone-400" />
 </div>
 <div>
 <p className="text-sm font-bold text-stone-900">Add New Gateway</p>
 <p className="text-[10px] text-stone-400">Connect to international banks or wallets</p>
 </div>
 </div>
 </div>
 </motion.div>
 ...
 <div className="bg-white rounded-lg p-10 flex flex-col md:flex-row gap-12 items-center justify-between relative overflow-hidden">
 ...
 <p className="text-sm text-stone-400 leading-relaxed">Hệ thống AI giám sát mọi giao dịch 24/7 để phát hiện các hành vi bất thường như rửa tiền, gian lận thẻ hoặc nạp tiền ảo. Tự động đóng băng tài khoản khi có rủi ro cao để bảo vệ tài sản của Doanh nghiệp.</p>
 </div>

 <div className="relative z-10 flex flex-col gap-3 min-w-[240px]">
 <button className="w-full py-4 bg-white text-stone-900 font-bold rounded-lg hover:bg-stone-100 transition-all shadow-sm text-sm flex items-center justify-center gap-2">
 Fraud Analysis Report <BarChart2 className="w-4 h-4" />
 </button>
 <button className="w-full py-4 bg-white/5 text-[#FAF9F5] font-bold rounded-lg hover:bg-white/10 transition-all border border-white/10 text-sm flex items-center justify-center gap-2">
