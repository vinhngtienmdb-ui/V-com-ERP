...
export function WorkflowHub() {
 ...
 <span className="text-[10px] tracking-[0.2em] text-orange-700 bg-slate-100 px-2 py-0.5 rounded">Mission Control Hub</span>
 <div className="w-1.5 h-1.5 bg-slate-800 rounded-full animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.6)]" />
 </div>
 <h1 className="font-sans tracking-tight text-3xl font-semibold text-slate-900 tracking-tight italic">Operations & <span className="text-orange-700 font-sans">Workflows</span></h1>
 ...
 className="bg-white border border-slate-300 px-5 py-2.5 rounded-lg text-[10px] hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm border-b-2 active:translate-y-0.5"
 >
 ...
 <button className="bg-slate-900 text-[#FAF9F5] px-5 py-2.5 rounded-lg text-[10px] hover:bg-slate-800 transition-all shadow-sm shadow-slate-900/10 flex items-center gap-2 hover:scale-[1.02] active:scale-95">
 ...
 className={cn(
 "px-6 py-4 text-[10px] transition-all rounded-lg flex items-center gap-2",
  ...
  <div className="bg-gradient-to-r from-orange-50/50 to-orange-100/50 p-6 rounded-lg border border-orange-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
   <div className="flex items-start gap-4">
    ...
    <div className="space-y-1 text-left">
     <h1 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Đồng bộ lịch Google Calendar</h1>
     {calendarSession ? (
    ...
    {calendarSession ? (
     <>
      <button 
       ...
       className="px-4 py-2.5 bg-slate-900 text-[#FAF9F5] text-[10px] rounded-lg hover:bg-slate-800 transition-all shadow-sm active:scale-95"
      >
       Đồng bộ tất cả
      </button>
      <button 
       onClick={handleDisconnectCalendar}
       className="px-4 py-2.5 bg-white border border-slate-300 text-rose-600 hover:text-rose-700 text-[10px] rounded-lg hover:bg-slate-50 transition-all shadow-sm active:scale-95"
      >
       Hủy liên kết
      </button>
     </>
     ...
     <button 
       ...
       className="px-5 py-2.5 bg-orange-700 text-[#FAF9F5] text-[10px] rounded-lg hover:bg-orange-850 transition-all shadow-md shadow-orange-700/10 flex items-center gap-2 active:scale-95"
     >
 ...
 className={cn("px-4 py-2 rounded-lg text-[10px] transition-all", filter === 'all' ? "bg-slate-900 text-[#FAF9F5]" : "bg-slate-50 text-slate-600 hover:bg-slate-100")}
 >
 Tất cả
 </button>
 <button 
 onClick={() => setFilter('critical')}
 className={cn("px-4 py-2 rounded-lg text-[10px] transition-all", filter === 'critical' ? "bg-rose-600 text-[#FAF9F5]" : "bg-slate-50 text-slate-600 hover:bg-slate-100")}
 >
 Critical
 </button>
 </div>
 ...
 <div className="overflow-x-auto min-w-0 custom-scrollbar-x border border-slate-200 rounded-lg">
  <table className="min-w-full w-max text-left border-collapse whitespace-nowrap bg-white">
   <thead className="bg-slate-50 border-b border-slate-200">
    <tr>
     <ResizableTh columnId="id" width={taskColumns.find(c=>c.id==='id')?.currentWidth || 120} onResize={(w) => handleTaskResize('id', w)} className="px-4 py-3 text-[10px] text-slate-500 whitespace-nowrap">ID / Module</ResizableTh>
     <ResizableTh columnId="title" width={taskColumns.find(c=>c.id==='title')?.currentWidth || 350} onResize={(w) => handleTaskResize('title', w)} className="px-4 py-3 text-[10px] text-slate-500">Nhiệm vụ</ResizableTh>
     <ResizableTh columnId="priority" width={taskColumns.find(c=>c.id==='priority')?.currentWidth || 120} onResize={(w) => handleTaskResize('priority', w)} className="px-4 py-3 text-[10px] text-slate-500 whitespace-nowrap text-center">Độ ưu tiên</ResizableTh>
     <ResizableTh columnId="deadline" width={taskColumns.find(c=>c.id==='deadline')?.currentWidth || 150} onResize={(w) => handleTaskResize('deadline', w)} className="px-4 py-3 text-[10px] text-slate-500 whitespace-nowrap">Hạn chót</ResizableTh>
     <ResizableTh columnId="actions" width={taskColumns.find(c=>c.id==='actions')?.currentWidth || 180} onResize={(w) => handleTaskResize('actions', w)} className="px-4 py-3 text-[10px] text-slate-500 whitespace-nowrap text-right">Thao tác</ResizableTh>
    </tr>
   </thead>
    ...
    {tasks.filter(t => filter === 'all' || t.priority === filter).map((task) => (
     <tr key={task.id} className="hover:bg-slate-50 transition-colors group">
      <td className="px-4 py-3">
       <span className="text-[10px] text-orange-700 block">{task.module}</span>
      ...
      <td className="px-4 py-3 text-center">
       {task.priority === 'critical' ? (
        <span className="px-2.5 py-1 bg-rose-50 text-rose-600 rounded text-[10px] flex items-center justify-center gap-1 w-fit mx-auto"><AlertCircle className="w-3 h-3" /> Critical</span>
       ) : task.priority === 'high' ? (
        <span className="px-2.5 py-1 bg-orange-50 text-orange-600 rounded text-[10px] flex items-center justify-center gap-1 w-fit mx-auto"><AlertCircle className="w-3 h-3" /> High</span>
       ) : (
        <span className="px-2.5 py-1 bg-blue-50 text-blue-600 rounded text-[10px] flex items-center justify-center gap-1 w-fit mx-auto"><Activity className="w-3 h-3" /> Medium</span>
       )}
      </td>
      <td className="px-4 py-3">
       <div className="flex flex-col gap-1">
        ...
        {syncedTaskIds.includes(task.id) && <span className="text-[9px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded w-fit flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Đã đồng bộ Lịch</span>}
       </div>
      </td>
 ...
 <h3 className="text-2xl font-semibold italic tracking-tight">AI Process <br/> <span className="text-orange-500">Accelerator</span></h3>
 ...
 <button className="w-full py-4 bg-white/5 border border-white/10 text-[#FAF9F5] rounded-lg text-[10px] hover:bg-white/10 transition-all">Audit Automation Log</button>
 </div>
 ...
 <h4 className="font-semibold text-slate-900 uppercase tracking-widest text-xs mb-8 flex items-center gap-3">
 ...
 <span className="text-[10px] text-slate-600 italic">Neural Canvas Active | Beta v2.4</span>
 </div>
 <div className="flex gap-2">
 <button className="px-5 py-2.5 bg-slate-100 text-slate-900 rounded-lg text-[10px] hover:bg-slate-200 transition-all">Save Draft</button>
 <button className="px-5 py-2.5 bg-emerald-600 text-[#FAF9F5] rounded-lg text-[10px] hover:bg-emerald-700 shadow-sm shadow-emerald-600/20 transition-all">Publish Flow</button>
 </div>
 </div>
 </div>
 )}
 ...
 <span className="text-[10px] tracking-[0.2em] text-emerald-600">Enterprise Digital Identity</span>
 </div>
 ...
 <h4 className="font-semibold text-sm uppercase tracking-tight">{ca.name}</h4>
 <p className={cn("text-[10px] mt-1 opacity-60", signatureMethod === ca.id ? "text-blue-300" : "text-slate-600")}>{ca.desc}</p>
 ...
 <p className="text-xs font-semibold text-slate-900 uppercase">Audit ID: {signingTaskId}</p>
 <p className="text-[10px] text-slate-500">TLS 1.3 Encryption Active</p>
 </div>
 </div>
 <button 
 ...
 className={cn(
 "px-6 py-5 rounded-lg text-[10px] tracking-[0.2em] transition-all shadow-sm flex items-center gap-3 w-full sm:w-auto justify-center active:translate-y-1",
   ...
   >
    ...
    <div className="p-6 space-y-6">
     <div className="flex items-center gap-4">
      ...
      <div>
       <span className="text-[10px] text-orange-700/80">Google Calendar Integration</span>
       <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Xác nhận đồng bộ Lịch</h2>
      </div>
     </div>
     ...
     <div className="bg-slate-50 p-5 rounded-lg border border-slate-200/60 text-left space-y-3">
      ...
      <div className="text-sm font-semibold text-slate-900 uppercase tracking-tight leading-snug">{syncingTask.title}</div>
      ...
      <div className="bg-orange-50/50 p-3 rounded-lg border border-orange-100 text-xs mt-2 font-semibold text-slate-800">
     ...
     <div className="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
      <button 
       onClick={() => setSyncingTask(null)}
       className="px-5 py-3 text-[10px] bg-slate-50 border border-slate-300 rounded-lg hover:bg-slate-100 transition-all text-slate-600"
       disabled={isSyncingInProgress}
      >
       Hủy
      </button>
      <button 
       ...
       disabled={isSyncingInProgress}
       className={cn(
        "px-5 py-3 rounded-lg text-[10px] text-[#FAF9F5] shadow-sm flex items-center gap-2",
   ...
   >
    ...
    <div className="p-6 space-y-6">
     <div className="flex items-center gap-4">
      ...
      <div>
       <span className="text-[10px] text-orange-700/80">Google Calendar Batch Process</span>
       <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Đồng bộ hàng loạt</h2>
      </div>
     </div>
     ...
     <div className="max-h-48 overflow-y-auto space-y-2 text-left">
      {tasks.filter(t => !syncedTaskIds.includes(t.id)).map(task => (
       <div key={task.id} className="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between text-xs">
        ...
        <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded text-slate-600 shrink-0">{task.deadline}</span>
       </div>
      ))}
     </div>
