...
export function Workspace() {
 ...
 <button onClick={() => setActiveModule('overview')} className="p-1 hover:bg-stone-100 rounded-md transition-colors mr-1">
 ...
 {/* News / Alert Banner */}
 <div className="bg-stone-900 rounded-lg p-8 text-[#FAF9F5] relative overflow-hidden shadow-sm shadow-blue-900/10">
 <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start md:items-center">
 <div className="flex-1 space-y-4">
 <div className="flex items-center gap-3">
 <div className="bg-stone-800/20 text-orange-500 p-2 rounded-lg border border-stone-900/30">
 ...
 news.priority === 'medium' ? "bg-orange-500/20 text-orange-400" : "bg-stone-800/20 text-orange-500"
 )}>
 {news.type}
 </span>
 <span className="text-[10px] text-stone-400 font-medium">{news.date}</span>
 </div>
 <h3 className="text-sm font-bold text-stone-100 group-hover:text-orange-500 transition-colors line-clamp-2 leading-relaxed">
 {news.title}
 </h3>
 </div>
 ))}
 ...
 className="bg-stone-50/50 rounded-lg border border-stone-200 flex flex-col min-h-[600px]"
 >
 ...
 <span className="bg-white border border-stone-200 text-stone-500 text-[10px] font-bold px-1.5 py-0.5 rounded-full">
 {tasks.filter(t => t.status === status).length}
 </span>
 </div>
 <button className="p-1 hover:bg-white rounded transition-colors text-stone-400"><Plus className="w-4 h-4" /></button>
 </div>
 ...
 className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm hover:shadow-sm hover:border-blue-300 transition-all cursor-grab active:cursor-grabbing group"
 >
 <div className="flex justify-between items-start mb-2">
 <span className={cn(
 "text-[9px] font-bold px-1.5 py-0.5 rounded uppercase",
 task.priority === 'Gấp' ? "bg-rose-50 text-rose-600" : "bg-[#F2F0E9] text-orange-700"
 )}>
 {task.priority}
 </span>
 <button className="opacity-0 group-hover:opacity-100 p-1 hover:bg-stone-50 rounded transition-opacity"><ArrowUpRight className="w-3 h-3 text-stone-400" /></button>
 </div>
 <h4 className="text-sm font-bold text-stone-800 group-hover:text-orange-700 transition-colors mb-2">{task.title}</h4>
 {task.desc && <p className="text-[11px] text-stone-500 line-clamp-2 mb-3 leading-relaxed">{task.desc}</p>}
 <div className="flex items-center justify-between mt-4 pt-3 border-t border-stone-50">
 <span className="flex items-center gap-1 text-[10px] font-bold text-stone-400"><Clock className="w-3 h-3" /> {task.date}</span>
 <div className="flex -space-x-1.5">
 {[1, 2].map(i => (
 <div key={i} className="w-5 h-5 rounded-full border-2 border-white bg-stone-100 flex items-center justify-center text-[8px] font-bold text-stone-500 overflow-hidden">
