...
export const QuickActionCard: React.FC<QuickActionCardProps> = ({ title, description, icon: Icon, onClick, color }) => (
  <button
    ...
    className={cn(
      ...
      color === 'bg-slate-900'
        ? 'bg-white border-slate-900 hover:border-slate-700'
        : color === 'bg-emerald-600'
          ? 'bg-white border-emerald-500 hover:border-emerald-600'
          : 'bg-white border-slate-300 hover:border-slate-400'
    )}
  >
    <div className="absolute inset-0 bg-white/0 group-hover:bg-slate-50/50 transition-colors duration-300 pointer-events-none" />
